...
int LoadZIMPtr(const uint8_t *zim, size_t datasize, int *width, int *height, int *flags, uint8_t **image) {
	...
	for (int i = 0; i < num_levels; i++) {
		...
		case ZIM_RGB565:
			...
			break;
		default:
			printf("Invalid ZIM format %i", *flags & ZIM_FORMAT_MASK);
			return 0;
		}
	...
	if (total_data_size == 0) {
		printf("Invalid ZIM data size 0");
		return 0;
	}
	...
	if (*flags & ZIM_ZLIB_COMPRESSED) {
		...
		if (Z_OK != retcode) {
			printf("ZIM zlib format decompression failed: %d", retcode);
			free(*image);
			*image = 0;
			return 0;
		}
		if (outlen != total_data_size) {
			...
			printf("Wrong size data in ZIM: %i vs %i", (int)outlen, (int)total_data_size);
		}
	} else {
		memcpy(*image, zim + 16, datasize - 16);
		if (datasize - 16 != (size_t)total_data_size) {
			printf("Wrong size data in ZIM: %i vs %i", (int)(datasize-16), (int)total_data_size);
		}
	}
	return num_levels;
}
...
int LoadZIM(const char *filename, int *width, int *height, int *format, uint8_t **image) {
	size_t size;
	uint8_t *buffer = SCREEN_VFSReadFile(filename, &size);
	if (!buffer) {
		printf("Couldn't read data for '%s'", buffer);
		return 0;
	}

	int retval = LoadZIMPtr(buffer, (int)size, width, height, format, image);
	if (!retval) {
		printf("Not a valid ZIM file: %s (size: %d bytes)", filename, (int)size);
	}
	delete [] buffer;
	return retval;
}
