...
SCREEN_KeyDef AxisDef(int deviceId, int axisId, int direction);
...
void UpdateNativeMenuKeys() {
	std::vector<SCREEN_KeyDef> SCREEN_confirmKeys, SCREEN_cancelKeys;
	std::vector<SCREEN_KeyDef> tabLeft, tabRight;
	std::vector<SCREEN_KeyDef> upKeys, downKeys, leftKeys, rightKeys;

	int confirmKey = CTRL_CROSS;
	int cancelKey = CTRL_CIRCLE;

	// Mouse mapping might be problematic in UI, so let's ignore mouse for UI
	KeyFromPspButton(confirmKey, &SCREEN_confirmKeys, true);
	KeyFromPspButton(cancelKey, &SCREEN_cancelKeys, true);
	...
	for (size_t i = 0; i < ARRAY_SIZE(hardcodedConfirmKeys); i++) {
		if (std::find(SCREEN_confirmKeys.begin(), SCREEN_confirmKeys.end(), hardcodedConfirmKeys[i]) == SCREEN_confirmKeys.end())
			SCREEN_confirmKeys.push_back(hardcodedConfirmKeys[i]);
	}

	const SCREEN_KeyDef hardcodedCancelKeys[] = {
		SCREEN_KeyDef(DEVICE_ID_KEYBOARD, NKCODE_ESCAPE),
		SCREEN_KeyDef(DEVICE_ID_ANY, NKCODE_BACK),
		SCREEN_KeyDef(DEVICE_ID_ANY, NKCODE_BUTTON_B),
		SCREEN_KeyDef(DEVICE_ID_MOUSE, NKCODE_EXT_MOUSEBUTTON_4),
	};

	for (size_t i = 0; i < ARRAY_SIZE(hardcodedCancelKeys); i++) {
		if (std::find(SCREEN_cancelKeys.begin(), SCREEN_cancelKeys.end(), hardcodedCancelKeys[i]) == SCREEN_cancelKeys.end())
			SCREEN_cancelKeys.push_back(hardcodedCancelKeys[i]);
	}

	SetDPadKeys(upKeys, downKeys, leftKeys, rightKeys);
	SetConfirmCancelKeys(SCREEN_confirmKeys, SCREEN_cancelKeys);
...
static void SetDefaultKeyMap(int deviceId, const DefMappingStruct *array, size_t count, bool replace) {
	for (size_t i = 0; i < count; i++) {
		if (array[i].direction == 0)
			SetKeyMapping(array[i].pspKey, SCREEN_KeyDef(deviceId, array[i].key), replace);
		else
...
SCREEN_KeyDef AxisDef(int deviceId, int axisId, int direction) {
	return SCREEN_KeyDef(deviceId, TranslateKeyCodeFromAxis(axisId, direction));
}
...
static bool FindKeyMapping(int deviceId, int key, std::vector<int> *psp_button) {
	...
	for (auto iter = g_controllerMap.begin(); iter != g_controllerMap.end(); ++iter) {
		for (auto iter2 = iter->second.begin(); iter2 != iter->second.end(); ++iter2) {
			if (*iter2 == SCREEN_KeyDef(deviceId, key)) {
...
void SetKeyMapping(int btn, SCREEN_KeyDef key, bool replace) {
	if (key.keyCode < 0)
		return;
	if (replace) {
...
void SetAxisMapping(int btn, int deviceId, int axisId, int direction, bool replace) {
	int key = TranslateKeyCodeFromAxis(axisId, direction);
	SetKeyMapping(btn, SCREEN_KeyDef(deviceId, key), replace);
}
...
void LoadFromIni(SCREEN_IniFile &file) {
	RestoreDefault();
	if (!file.HasSection("ControlMapping")) {
		return;
	}

	SCREEN_Section *controls = file.GetOrCreateSection("ControlMapping");
	for (size_t i = 0; i < ARRAY_SIZE(psp_button_names); i++) {
		std::string value;
		...
		for (size_t j = 0; j < mappings.size(); j++) {
			...
			SetKeyMapping(psp_button_names[i].key, SCREEN_KeyDef(deviceId, keyCode), false);
		}
	}
...
void SaveToIni(SCREEN_IniFile &file) {
	SCREEN_Section *controls = file.GetOrCreateSection("ControlMapping");

	for (size_t i = 0; i < ARRAY_SIZE(psp_button_names); i++) {
		std::vector<SCREEN_KeyDef> keys;
...
void AutoConfForPad(const std::string &name) {
	printf("Autoconfiguring pad for '%s'", name.c_str());
	...
	g_controllerMap[VIRTKEY_PAUSE].push_back(SCREEN_KeyDef(DEVICE_ID_KEYBOARD, NKCODE_ESCAPE));
	g_controllerMap[VIRTKEY_UNTHROTTLE].push_back(SCREEN_KeyDef(DEVICE_ID_KEYBOARD, NKCODE_TAB));
	g_controllerMapGeneration++;
}
