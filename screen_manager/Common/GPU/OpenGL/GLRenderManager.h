...
public:
	...
	// Must ONLY be called from SCREEN_GLQueueRunner!
...
enum class SCREEN_GLBufferStrategy {
	SUBDATA = 0,
...
static inline int operator &(const SCREEN_GLBufferStrategy &lhs, const SCREEN_GLBufferStrategy &rhs) {
	return (int)lhs & (int)rhs;
}

class SCREEN_GLRBuffer {
public:
	SCREEN_GLRBuffer(GLuint target, size_t size) : target_(target), size_((int)size) {}
	~SCREEN_GLRBuffer() {
		if (buffer_) {
	...
	void *Map(SCREEN_GLBufferStrategy strategy);
	bool Unmap();
...
public:
	...
	struct BufInfo {
		SCREEN_GLRBuffer *buffer = nullptr;
		uint8_t *localMemory = nullptr;
		uint8_t *deviceMemory = nullptr;
		size_t flushOffset = 0;
	};

	SCREEN_GLPushBuffer(SCREEN_GLRenderManager *render, GLuint target, size_t size);
	~SCREEN_GLPushBuffer();
...
public:
	...
	size_t Allocate(size_t numBytes, SCREEN_GLRBuffer **vkbuf) {
		size_t out = offset_;
	...
	size_t Push(const void *data, size_t size, SCREEN_GLRBuffer **vkbuf) {
		_dbg_assert_(writePtr_);
	...
	uint32_t PushAligned(const void *data, size_t size, int align, SCREEN_GLRBuffer **vkbuf) {
		_dbg_assert_(writePtr_);
	...
	// Recommended.
	void *Push(size_t size, uint32_t *bindOffset, SCREEN_GLRBuffer **vkbuf) {
		_dbg_assert_(writePtr_);
		size_t off = Allocate(size, vkbuf);
		*bindOffset = (uint32_t)off;
		return writePtr_ + off;
	}
	void *PushAligned(size_t size, uint32_t *bindOffset, SCREEN_GLRBuffer **vkbuf, int align) {
		_dbg_assert_(writePtr_);
		offset_ = (offset_ + align - 1) & ~(align - 1);
		size_t off = Allocate(size, vkbuf);
		*bindOffset = (uint32_t)off;
...
protected:
	void MapDevice(SCREEN_GLBufferStrategy strategy);
	void UnmapDevice();
...
private:
	...
	GLuint target_;
	SCREEN_GLBufferStrategy strategy_ = SCREEN_GLBufferStrategy::SUBDATA;
};
...
public:
	...
	void Take(SCREEN_GLDeleter &other);

	std::vector<SCREEN_GLRShader *> shaders;
	std::vector<SCREEN_GLRProgram *> programs;
	std::vector<SCREEN_GLRBuffer *> buffers;
	std::vector<SCREEN_GLRTexture *> textures;
	std::vector<SCREEN_GLRInputLayout *> inputLayouts;
	std::vector<SCREEN_GLRFramebuffer *> framebuffers;
	std::vector<SCREEN_GLPushBuffer *> pushBuffers;
};
...
public:
	...
	SCREEN_GLRTexture *CreateTexture(GLenum target) {
		GLRInitStep step{ SCREEN_GLRInitStepType::CREATE_TEXTURE };
		step.create_texture.texture = new SCREEN_GLRTexture();
		step.create_texture.texture->target = target;
		initSteps_.push_back(step);
		return step.create_texture.texture;
	}

	SCREEN_GLRBuffer *CreateBuffer(GLuint target, size_t size, GLuint usage) {
		GLRInitStep step{ SCREEN_GLRInitStepType::CREATE_BUFFER };
		step.create_buffer.buffer = new SCREEN_GLRBuffer(target, size);
	...
	SCREEN_GLRShader *CreateShader(GLuint stage, const std::string &code, const std::string &desc) {
		GLRInitStep step{ SCREEN_GLRInitStepType::CREATE_SHADER };
		step.create_shader.shader = new SCREEN_GLRShader();
	...
	SCREEN_GLRFramebuffer *CreateFramebuffer(int width, int height, bool z_stencil) {
		GLRInitStep step{ SCREEN_GLRInitStepType::CREATE_FRAMEBUFFER };
		step.create_framebuffer.framebuffer = new SCREEN_GLRFramebuffer(width, height, z_stencil);
		initSteps_.push_back(step);
	...
	SCREEN_GLRProgram *CreateProgram(
		std::vector<SCREEN_GLRShader *> shaders, std::vector<SCREEN_GLRProgram::Semantic> semantics, std::vector<SCREEN_GLRProgram::UniformLocQuery> queries,
		std::vector<SCREEN_GLRProgram::Initializer> initalizers, bool supportDualSource) {
		GLRInitStep step{ SCREEN_GLRInitStepType::CREATE_PROGRAM };
		_assert_(shaders.size() <= ARRAY_SIZE(step.create_program.shaders));
		step.create_program.program = new SCREEN_GLRProgram();
	...
	SCREEN_GLRInputLayout *CreateInputLayout(std::vector<SCREEN_GLRInputLayout::Entry> &entries) {
		GLRInitStep step{ SCREEN_GLRInitStepType::CREATE_INPUT_LAYOUT };
		step.create_input_layout.inputLayout = new SCREEN_GLRInputLayout();
	...
	SCREEN_GLPushBuffer *CreatePushBuffer(int frame, GLuint target, size_t size) {
		SCREEN_GLPushBuffer *push = new SCREEN_GLPushBuffer(this, target, size);
		RegisterPushBuffer(frame, push);
		return push;
	}

	void DeleteShader(SCREEN_GLRShader *shader) {
		deleter_.shaders.push_back(shader);
	}
	void DeleteProgram(SCREEN_GLRProgram *program) {
		deleter_.programs.push_back(program);
	}
	void DeleteBuffer(SCREEN_GLRBuffer *buffer) {
		deleter_.buffers.push_back(buffer);
	}
	void DeleteTexture(SCREEN_GLRTexture *texture) {
		deleter_.textures.push_back(texture);
	}
	void DeleteInputLayout(SCREEN_GLRInputLayout *inputLayout) {
		deleter_.inputLayouts.push_back(inputLayout);
	}
	void DeleteFramebuffer(SCREEN_GLRFramebuffer *framebuffer) {
		deleter_.framebuffers.push_back(framebuffer);
	}
	void DeletePushBuffer(SCREEN_GLPushBuffer *pushbuffer) {
		deleter_.pushBuffers.push_back(pushbuffer);
	}

	void BeginPushBuffer(SCREEN_GLPushBuffer *pushbuffer) {
		pushbuffer->Begin();
	}

	void EndPushBuffer(SCREEN_GLPushBuffer *pushbuffer) {
		pushbuffer->End();
	}
	...
	void BindFramebufferAsRenderTarget(SCREEN_GLRFramebuffer *fb, SCREEN_GLRRenderPassAction color, SCREEN_GLRRenderPassAction depth, SCREEN_GLRRenderPassAction stencil, uint32_t clearColor, float clearDepth, uint8_t clearStencil, const char *tag);

	// Binds a framebuffer as a texture, for the following draws.
	void BindFramebufferAsTexture(SCREEN_GLRFramebuffer *fb, int binding, int aspectBit, int attachment);

	bool CopyFramebufferToMemorySync(SCREEN_GLRFramebuffer *src, int aspectBits, int x, int y, int w, int h, SCREEN_Draw::SCREEN_DataFormat destFormat, uint8_t *pixels, int pixelStride, const char *tag);
	void CopyImageToMemorySync(SCREEN_GLRTexture *texture, int mipLevel, int x, int y, int w, int h, SCREEN_Draw::SCREEN_DataFormat destFormat, uint8_t *pixels, int pixelStride, const char *tag);

	void CopyFramebuffer(SCREEN_GLRFramebuffer *src, GLRect2D srcRect, SCREEN_GLRFramebuffer *dst, GLOffset2D dstPos, int aspectMask, const char *tag);
	void BlitFramebuffer(SCREEN_GLRFramebuffer *src, GLRect2D srcRect, SCREEN_GLRFramebuffer *dst, GLRect2D dstRect, int aspectMask, bool filter, const char *tag);

	// Takes ownership of data if deleteData = true.
	void BufferSubdata(SCREEN_GLRBuffer *buffer, size_t offset, size_t size, uint8_t *data, bool deleteData = true) {
		...
		GLRInitStep step{ SCREEN_GLRInitStepType::BUFFER_SUBDATA };
	...
	void TextureImage(SCREEN_GLRTexture *texture, int level, int width, int height, SCREEN_Draw::SCREEN_DataFormat format, uint8_t *data, SCREEN_GLRAllocType allocType = SCREEN_GLRAllocType::NEW, bool linearFilter = false) {
		GLRInitStep step{ SCREEN_GLRInitStepType::TEXTURE_IMAGE };
	...
	void TextureSubImage(SCREEN_GLRTexture *texture, int level, int x, int y, int width, int height, SCREEN_Draw::SCREEN_DataFormat format, uint8_t *data, SCREEN_GLRAllocType allocType = SCREEN_GLRAllocType::NEW) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData _data{ SCREEN_GLRRenderCommand::TEXTURE_SUBIMAGE };
	...
	void FinalizeTexture(SCREEN_GLRTexture *texture, int maxLevels, bool genMips) {
		GLRInitStep step{ SCREEN_GLRInitStepType::TEXTURE_FINALIZE };
	...
	void BindTexture(int slot, SCREEN_GLRTexture *tex) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::BINDTEXTURE };
	...
	void BindProgram(SCREEN_GLRProgram *program) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::BINDPROGRAM };
	...
	void BindPixelPackBuffer(SCREEN_GLRBuffer *buffer) {  // Want to support an offset but can't in ES 2.0. We supply an offset when binding the buffers instead.
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::BIND_BUFFER };
		data.bind_buffer.buffer = buffer;
		data.bind_buffer.target = GL_PIXEL_PACK_BUFFER;
		curRenderStep_->commands.push_back(data);
	}

	void BindIndexBuffer(SCREEN_GLRBuffer *buffer) {  // Want to support an offset but can't in ES 2.0. We supply an offset when binding the buffers instead.
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::BIND_BUFFER};
		data.bind_buffer.buffer = buffer;
	...
	void BindVertexBuffer(SCREEN_GLRInputLayout *inputLayout, SCREEN_GLRBuffer *buffer, size_t offset) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		_dbg_assert_(inputLayout);
		GLRRenderData data{ SCREEN_GLRRenderCommand::BIND_VERTEX_BUFFER };
	...
	void SetDepth(bool enabled, bool write, GLenum func) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::DEPTH };
	...
	void SetViewport(const GLRViewport &vp) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::VIEWPORT };
		data.viewport.vp = vp;
		curRenderStep_->commands.push_back(data);
	}
	...
	void SetScissor(const GLRect2D &rc) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::SCISSOR };
		data.scissor.rc = rc;
		curRenderStep_->commands.push_back(data);
	}
...
#endif
		GLRRenderData data{ SCREEN_GLRRenderCommand::UNIFORM4I };
	...
	void SetUniformF(const GLint *loc, int count, const float *udata) {
		...
		_dbg_assert_(curProgram_);
#endif
		GLRRenderData data{ SCREEN_GLRRenderCommand::UNIFORM4F };
		data.uniform4.loc = loc;
		data.uniform4.count = count;
	...
	void SetUniformM4x4(const GLint *loc, const float *udata) {
		...
		_dbg_assert_(curProgram_);
#endif
		GLRRenderData data{ SCREEN_GLRRenderCommand::UNIFORMMATRIX };
	...
	void SetBlendAndMask(int colorMask, bool blendEnabled, GLenum srcColor, GLenum dstColor, GLenum srcAlpha, GLenum dstAlpha, GLenum funcColor, GLenum funcAlpha) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::BLEND };
	...
	void SetLogicOp(bool enabled, GLenum logicOp) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::LOGICOP };
	...
	void SetStencilFunc(bool enabled, GLenum func, uint8_t refValue, uint8_t compareMask) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::STENCILFUNC };
	...
	void SetStencilOp(uint8_t writeMask, GLenum sFail, GLenum zFail, GLenum pass) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::STENCILOP };
	...
	void SetStencilDisabled() {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data;
		data.cmd = SCREEN_GLRRenderCommand::STENCILFUNC;
	...
	void SetBlendFactor(const float color[4]) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::BLENDCOLOR };
	...
	void SetRaster(GLboolean cullEnable, GLenum frontFace, GLenum cullFace, GLboolean ditherEnable) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::RASTER };
	...
	void SetTextureSampler(int slot, GLenum wrapS, GLenum wrapT, GLenum magFilter, GLenum minFilter, float anisotropy) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::TEXTURESAMPLER };
	...
	void SetTextureLod(int slot, float minLod, float maxLod, float lodBias) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::TEXTURELOD};
	...
	void Clear(uint32_t clearColor, float clearZ, int clearStencil, int clearMask, int colorMask, int scissorX, int scissorY, int scissorW, int scissorH) {
		...
		if (!clearMask)
			return;
		GLRRenderData data{ SCREEN_GLRRenderCommand::CLEAR };
	...
	void Draw(GLenum mode, int first, int count) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::DRAW };
		data.draw.mode = mode;
	...
	void DrawIndexed(GLenum mode, int count, GLenum indexType, void *indices, int instances = 1) {
		_dbg_assert_(curRenderStep_ && curRenderStep_->stepType == SCREEN_GLRStepType::RENDER);
		GLRRenderData data{ SCREEN_GLRRenderCommand::DRAW_INDEXED };
	...
	void UnregisterPushBuffer(SCREEN_GLPushBuffer *buffer) {
		int foundCount = 0;
...
private:
	...
	void RegisterPushBuffer(int frame, SCREEN_GLPushBuffer *buffer) {
	...
	struct FrameData {
		...
		SCREEN_GLDeleter deleter;
		SCREEN_GLDeleter deleter_prev;
		std::set<SCREEN_GLPushBuffer *> activePushBuffers;
	};
	...
	SCREEN_GLQueueRunner queueRunner_;
	...
	SCREEN_GLBufferStrategy bufferStrategy_ = SCREEN_GLBufferStrategy::SUBDATA;
...
#ifdef _DEBUG
	SCREEN_GLRProgram *curProgram_ = nullptr;
#endif
};
