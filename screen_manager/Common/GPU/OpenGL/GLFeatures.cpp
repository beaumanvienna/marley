...
void ProcessGPUFeatures() {
	...
	printf("Checking for GL driver bugs... vendor=%i model='%s'", (int)gl_extensions.gpuVendor, gl_extensions.model);
	...
	if (gl_extensions.gpuVendor == GPU_VENDOR_IMGTEC) {
		if (!strcmp(gl_extensions.model, "PowerVR SGX 545") ||
			...
			printf("GL DRIVER BUG: PVR with bad and terrible precision");
			...
			printf("GL DRIVER BUG: PVR with bad precision");
...
void CheckGLExtensions() {
	...
	printf("GPU Vendor : %s ; renderer: %s version str: %s ; GLSL version str: %s", cvendor, renderer ? renderer : "N/A", versionStr ? versionStr : "N/A", glslVersionStr ? glslVersionStr : "N/A");
	...
	if (!gl_extensions.IsGLES) { // For desktop GL
		...
		if (glGetError() != GL_NO_ERROR) {
			...
			printf("GL ES version mismatch. Version string '%s' parsed as %d.%d but API return %d.%d. Fallback to GL ES 2.0.", 
		...
		if (gl_extensions.GLES3) {
			if (gl_extensions.ver[1] >= 1) {
				printf("OpenGL ES 3.1 support detected!\n");
			} else {
				printf("OpenGL ES 3.0 support detected!\n");
			}
		}
	}
	...
	if (gl_extensions.IsGLES) {
		...
		printf("Addresses returned for invalid extensions: %p %p", invalidAddress, invalidAddress2);
#endif
...
#if defined(USING_GLES2) && defined(__linux__)
	...
	if (gl_extensions.IsGLES || g_set_gl_extensions.count("GL_ARB_ES2_compatibility") || gl_extensions.VersionGEThan(4, 1)) {
		...
		if (gl_extensions.gpuVendor == GPU_VENDOR_QUALCOMM) {
			printf("Detected Adreno - lowering int precision");
	...
	if (error)
		printf("GL error in init: %i", error);
