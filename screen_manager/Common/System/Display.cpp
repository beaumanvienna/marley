...
SCREEN_DisplayRotation g_display_rotation;
...
void RotateRectToDisplayImpl(DisplayRect<T> &rect, T curRTWidth, T curRTHeight) {
	...
	case SCREEN_DisplayRotation::ROTATE_180:
		rect.x = curRTWidth - rect.w - rect.x;
		rect.y = curRTHeight - rect.h - rect.y;
		break;
	case SCREEN_DisplayRotation::ROTATE_90: {
	...
	case SCREEN_DisplayRotation::ROTATE_270: {
		T origX = rect.x;
		T origY = rect.y;
		T rtw = curRTHeight;
		rect.x = origY;
	...
	case SCREEN_DisplayRotation::ROTATE_0:
	default:
		break;
	}
}
