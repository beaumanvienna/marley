    ...
    : BluetoothBase(ios, device_name)
{
  ...
  for (u8 i = 0; i != MAX_BBMOTES; ++i)
  {
    ...
    DEBUG_LOG(IOS_WIIMOTE, "Wii Remote {} BT ID {:x},{:x},{:x},{:x},{:x},{:x}", i, tmp_bd[0],
...
IPCCommandResult BluetoothEmu::IOCtlV(const IOCtlVRequest& request)
{
  ...
  case USB::IOCTLV_USBV0_BLKMSG:
  {
    ...
    case ACL_DATA_IN:  // We are given an ACL buffer to fill
    {
      ...
      DEBUG_LOG(IOS_WIIMOTE, "ACL_DATA_IN: {:#010x}", request.address);
      send_reply = false;
      break;
    }
    default:
  ...
  case USB::IOCTLV_USBV0_INTRMSG:
  {
    ...
    if (ctrl.endpoint == HCI_EVENT)  // We are given a HCI buffer to fill
    {
      ...
      DEBUG_LOG(IOS_WIIMOTE, "HCI_EVENT: {:#010x}", request.address);
      send_reply = false;
    }
    else
    {
...
void BluetoothEmu::SendToDevice(u16 connection_handle, u8* data, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "Send ACL Packet to ConnectionHandle {:#06x}", connection_handle);
...
{
  DEBUG_LOG(IOS_WIIMOTE, "HCI event {:x} completed...",
  ...
  if (m_hci_endpoint)
  {
    ...
    {
      DEBUG_LOG(IOS_WIIMOTE, "HCI endpoint not currently valid, queueing ({})...",
                    m_event_queue.size());
  ...
  {
    DEBUG_LOG(IOS_WIIMOTE, "HCI endpoint not currently valid, queuing ({})...",
                  m_event_queue.size());
    m_event_queue.push_back(event);
  }
}
...
void BluetoothEmu::Update()
{
  ...
  if (!m_event_queue.empty() && m_hci_endpoint)
  {
    ...
    DEBUG_LOG(IOS_WIIMOTE, "HCI event {:x} being written from queue ({}) to {:08x}...",
...
void BluetoothEmu::ACLPool::WriteToEndpoint(const USB::V0BulkMessage& endpoint)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "ACL packet being written from queue to {:08x}",
...
bool BluetoothEmu::SendEventConnectionComplete(const bdaddr_t& bd, u8 status)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  Connection_Handle: {:#06x}",
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  LinkType: {}", link_type[connection_complete->LinkType]);
  DEBUG_LOG(IOS_WIIMOTE, "  EncryptionEnabled: {}", connection_complete->EncryptionEnabled);
...
bool BluetoothEmu::SendEventRequestConnection(const WiimoteDevice& wiimote)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  COD[0]: {:#04x}", event_request_connection->uclass[0]);
  DEBUG_LOG(IOS_WIIMOTE, "  COD[1]: {:#04x}", event_request_connection->uclass[1]);
  DEBUG_LOG(IOS_WIIMOTE, "  COD[2]: {:#04x}", event_request_connection->uclass[2]);
  DEBUG_LOG(IOS_WIIMOTE, "  LinkType: {}", link_type[event_request_connection->LinkType]);
...
bool BluetoothEmu::SendEventDisconnect(u16 connection_handle, u8 reason)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  Connection_Handle: {:#06x}", disconnect->Connection_Handle);
  DEBUG_LOG(IOS_WIIMOTE, "  Reason: {:#04x}", disconnect->Reason);
...
bool BluetoothEmu::SendEventRemoteNameReq(const bdaddr_t& bd)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  RemoteName: {}", remote_name_req->RemoteName);
...
bool BluetoothEmu::SendEventReadRemoteVerInfo(u16 connection_handle)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  lmp_version: {:#04x}", read_remote_ver_info->lmp_version);
  DEBUG_LOG(IOS_WIIMOTE, "  manufacturer: {:#06x}", read_remote_ver_info->manufacturer);
  DEBUG_LOG(IOS_WIIMOTE, "  lmp_subversion: {:#06x}", read_remote_ver_info->lmp_subversion);
...
void BluetoothEmu::SendEventCommandComplete(u16 opcode, const void* data, u32 data_size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "Event: Command Complete (Opcode: {:#06x})", hci_event->Opcode);
...
bool BluetoothEmu::SendEventCommandStatus(u16 opcode)
{
  ...
  INFO_LOG(IOS_WIIMOTE, "Event: Command Status (Opcode: {:#06x})", hci_event->Opcode);
...
bool BluetoothEmu::SendEventRoleChange(bdaddr_t bd, bool master)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  NewRole: {}", role_change->NewRole);
...
bool BluetoothEmu::SendEventNumberOfCompletedPackets()
{
  ...
  for (unsigned int i = 0; i < m_wiimotes.size(); i++)
  {
    ...
    DEBUG_LOG(IOS_WIIMOTE, "  Connection_Handle: {:#06x}", info->con_handle);
    DEBUG_LOG(IOS_WIIMOTE, "  Number_Of_Completed_Packets: {}", info->compl_pkts);
...
bool BluetoothEmu::SendEventModeChange(u16 connection_handle, u8 mode, u16 value)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  Connection_Handle: {:#06x}", mode_change->Connection_Handle);
  DEBUG_LOG(IOS_WIIMOTE, "  Current Mode: {:#04x}", mode_change->CurrentMode = mode);
...
bool BluetoothEmu::SendEventReadClockOffsetComplete(u16 connection_handle)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ClockOffset: {:#06x}", read_clock_offset_complete->ClockOffset);
...
bool BluetoothEmu::SendEventConPacketTypeChange(u16 connection_handle, u16 packet_type)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  PacketType: {:#06x}", change_con_packet_type->PacketType);
...
void BluetoothEmu::ExecuteHCICommandMessage(const USB::V0CtrlMessage& ctrl_message)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "**************************************************");
  DEBUG_LOG(IOS_WIIMOTE, "Execute HCI Command: {:#06x} (ocf: {:#04x}, ogf: {:#04x})",
                msg.Opcode, ocf, ogf);
  ...
  default:
    ...
    {
      ERROR_LOG(IOS_WIIMOTE, "Command: vendor specific: {:#06x} (ocf: {:#x})", msg.Opcode, ocf);
...
void BluetoothEmu::CommandInquiry(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  LAP[0]: {:#04x}", inquiry.lap[0]);
  DEBUG_LOG(IOS_WIIMOTE, "  LAP[1]: {:#04x}", inquiry.lap[1]);
  DEBUG_LOG(IOS_WIIMOTE, "  LAP[2]: {:#04x}", inquiry.lap[2]);
  DEBUG_LOG(IOS_WIIMOTE, "  inquiry_length: {} (N x 1.28) sec", inquiry.inquiry_length);
  DEBUG_LOG(IOS_WIIMOTE, "  num_responses: {} (N x 1.28) sec", inquiry.num_responses);
...
void BluetoothEmu::CommandCreateCon(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  pkt_type: {}", create_connection.pkt_type);
  DEBUG_LOG(IOS_WIIMOTE, "  page_scan_rep_mode: {}", create_connection.page_scan_rep_mode);
  DEBUG_LOG(IOS_WIIMOTE, "  page_scan_mode: {}", create_connection.page_scan_mode);
  DEBUG_LOG(IOS_WIIMOTE, "  clock_offset: {}", create_connection.clock_offset);
  DEBUG_LOG(IOS_WIIMOTE, "  accept_role_switch: {}", create_connection.accept_role_switch);
...
void BluetoothEmu::CommandDisconnect(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#06x}", disconnect.con_handle);
  DEBUG_LOG(IOS_WIIMOTE, "  Reason: {:#04x}", disconnect.reason);
...
void BluetoothEmu::CommandAcceptCon(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  role: {}", roles[accept_connection.role]);
...
void BluetoothEmu::CommandChangeConPacketType(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#06x}", change_packet_type.con_handle);
  DEBUG_LOG(IOS_WIIMOTE, "  PacketType: {:#06x}", change_packet_type.pkt_type);
...
void BluetoothEmu::CommandAuthenticationRequested(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#06x}", auth_req.con_handle);
...
void BluetoothEmu::CommandRemoteNameReq(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  bd: {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                remote_name_req.bdaddr[0], remote_name_req.bdaddr[1], remote_name_req.bdaddr[2],
                remote_name_req.bdaddr[3], remote_name_req.bdaddr[4], remote_name_req.bdaddr[5]);
  DEBUG_LOG(IOS_WIIMOTE, "  page_scan_rep_mode: {}", remote_name_req.page_scan_rep_mode);
  DEBUG_LOG(IOS_WIIMOTE, "  page_scan_mode: {}", remote_name_req.page_scan_mode);
  DEBUG_LOG(IOS_WIIMOTE, "  clock_offset: {}", remote_name_req.clock_offset);
...
void BluetoothEmu::CommandReadRemoteFeatures(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#06x}", read_remote_features.con_handle);
...
void BluetoothEmu::CommandReadRemoteVerInfo(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#04x}", read_remote_ver_info.con_handle);
...
void BluetoothEmu::CommandReadClockOffset(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#04x}", read_clock_offset.con_handle);
...
void BluetoothEmu::CommandSniffMode(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#06x}", sniff_mode.con_handle);
  DEBUG_LOG(IOS_WIIMOTE, "  max_interval: {} msec", sniff_mode.max_interval * .625);
  DEBUG_LOG(IOS_WIIMOTE, "  min_interval: {} msec", sniff_mode.min_interval * .625);
  DEBUG_LOG(IOS_WIIMOTE, "  attempt: {} msec", sniff_mode.attempt * 1.25);
  DEBUG_LOG(IOS_WIIMOTE, "  timeout: {} msec", sniff_mode.timeout * 1.25);
...
void BluetoothEmu::CommandWriteLinkPolicy(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  ConnectionHandle: {:#06x}", link_policy.con_handle);
  DEBUG_LOG(IOS_WIIMOTE, "  Policy: {:#06x}", link_policy.settings);
...
void BluetoothEmu::CommandSetEventFilter(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  filter_type: {}", set_event_filter.filter_type);
  DEBUG_LOG(IOS_WIIMOTE, "  filter_condition_type: {}", set_event_filter.filter_condition_type);
...
void BluetoothEmu::CommandReadStoredLinkKey(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  read_all: {}", read_stored_link_key.read_all);
  DEBUG_LOG(IOS_WIIMOTE, "return:");
  DEBUG_LOG(IOS_WIIMOTE, "  max_num_keys: {}", reply.max_num_keys);
  DEBUG_LOG(IOS_WIIMOTE, "  num_keys_read: {}", reply.num_keys_read);
...
void BluetoothEmu::CommandDeleteStoredLinkKey(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  delete_all: {:#x}", delete_stored_link_key.delete_all);
...
void BluetoothEmu::CommandWriteLocalName(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  local_name: {}", write_local_name.name);
...
void BluetoothEmu::CommandWritePageTimeOut(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  timeout: {}", write_page_timeout.timeout);
...
void BluetoothEmu::CommandWriteScanEnable(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "Command: HCI_CMD_WRITE_SCAN_ENABLE: ({:#04x})",
                write_scan_enable.scan_enable);
  DEBUG_LOG(IOS_WIIMOTE, "  scan_enable: {}", scanning[write_scan_enable.scan_enable]);
...
void BluetoothEmu::CommandWriteUnitClass(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  COD[0]: {:#04x}", write_unit_class.uclass[0]);
  DEBUG_LOG(IOS_WIIMOTE, "  COD[1]: {:#04x}", write_unit_class.uclass[1]);
  DEBUG_LOG(IOS_WIIMOTE, "  COD[2]: {:#04x}", write_unit_class.uclass[2]);
...
void BluetoothEmu::CommandHostBufferSize(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  max_acl_size: {}", host_buffer_size.max_acl_size);
  DEBUG_LOG(IOS_WIIMOTE, "  max_sco_size: {}", host_buffer_size.max_sco_size);
  DEBUG_LOG(IOS_WIIMOTE, "  num_acl_pkts: {}", host_buffer_size.num_acl_pkts);
  DEBUG_LOG(IOS_WIIMOTE, "  num_sco_pkts: {}", host_buffer_size.num_sco_pkts);
...
void BluetoothEmu::CommandWriteLinkSupervisionTimeout(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  con_handle: {:#06x}", supervision.con_handle);
  DEBUG_LOG(IOS_WIIMOTE, "  timeout: {:#04x}", supervision.timeout);
...
void BluetoothEmu::CommandWriteInquiryScanType(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  type: {}", set_event_filter.type);
...
void BluetoothEmu::CommandWriteInquiryMode(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  mode: {}", inquiry_mode_tag[inquiry_mode.mode]);
...
void BluetoothEmu::CommandWritePageScanType(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  type: {}", page_scan_type[write_page_scan_type.type]);
...
void BluetoothEmu::CommandReadLocalVer(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "return:");
  DEBUG_LOG(IOS_WIIMOTE, "  status:         {}", reply.status);
  DEBUG_LOG(IOS_WIIMOTE, "  hci_revision:   {}", reply.hci_revision);
  DEBUG_LOG(IOS_WIIMOTE, "  lmp_version:    {}", reply.lmp_version);
  DEBUG_LOG(IOS_WIIMOTE, "  manufacturer:   {}", reply.manufacturer);
  DEBUG_LOG(IOS_WIIMOTE, "  lmp_subversion: {}", reply.lmp_subversion);
...
void BluetoothEmu::CommandReadBufferSize(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "return:");
  DEBUG_LOG(IOS_WIIMOTE, "  max_acl_size: {}", reply.max_acl_size);
  DEBUG_LOG(IOS_WIIMOTE, "  num_acl_pkts: {}", reply.num_acl_pkts);
  DEBUG_LOG(IOS_WIIMOTE, "  max_sco_size: {}", reply.max_sco_size);
  DEBUG_LOG(IOS_WIIMOTE, "  num_sco_pkts: {}", reply.num_sco_pkts);
...
void BluetoothEmu::CommandReadBDAdrr(const u8* input)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "return:");
  DEBUG_LOG(IOS_WIIMOTE, "  bd: {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}", reply.bdaddr[0],
...
void BluetoothEmu::CommandVendorSpecific_FC4F(const u8* input, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "Input (size {:#x}):", size);
...
WiimoteDevice* BluetoothEmu::AccessWiimote(u16 connection_handle)
{
  ...
  PanicAlertT("Can't find Wii Remote by connection handle {0:02x}", connection_handle);
