...
{
  INFO_LOG(IOS_WIIMOTE, "Wiimote: #{} Constructed", number);
...
void WiimoteDevice::ExecuteL2capCmd(u8* ptr, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "  CID {:#06x}, Len {:#x}, DataSize {:#x}", header->dcid,
  ...
  {
    ERROR_LOG(IOS_WIIMOTE, "L2CAP: SendACLPacket to unknown channel {}", header->dcid);
    return;
  }
  ...
  case L2CAP_PSM_HID_CNTL:
  {
    ...
    {
      ERROR_LOG(IOS_WIIMOTE, "Unknown HID-type ({:#x}) on L2CAP_PSM_HID_CNTL", hid_type);
    }
  }
  break;
  ...
  case L2CAP_PSM_HID_INTR:
  {
    ...
    if (hid_type == ((WiimoteCommon::HID_TYPE_DATA << 4) | WiimoteCommon::HID_PARAM_OUTPUT))
      ...
      ERROR_LOG(IOS_WIIMOTE, "Unknown HID-type ({:#x}) on L2CAP_PSM_HID_INTR", hid_type);
  }
  break;

  default:
    ERROR_LOG(IOS_WIIMOTE, "Channel {:#x} has unknown PSM {:x}", header->dcid, channel.psm);
    break;
  }
}
...
void WiimoteDevice::SignalChannel(u8* data, u32 size)
{
  while (size >= sizeof(l2cap_cmd_hdr_t))
  {
    ...
    default:
      ERROR_LOG(IOS_WIIMOTE, "  Unknown Command-Code ({:#04x})", cmd_hdr->code);
      return;
    }
...
void WiimoteDevice::ReceiveConnectionReq(u8 ident, u8* data, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    Ident: {:#04x}", ident);
  DEBUG_LOG(IOS_WIIMOTE, "    PSM: {:#06x}", command_connection_req->psm);
  DEBUG_LOG(IOS_WIIMOTE, "    SCID: {:#06x}", command_connection_req->scid);
  ...
  {
    ERROR_LOG(IOS_WIIMOTE, "Multiple channels with same PSM ({}) are not allowed.",
  ...
  else
  {
    ...
    {
      WARN_LOG(IOS_WIIMOTE, "L2CAP connection with unknown psm ({:#x})", channel.psm);
    }
...
void WiimoteDevice::ReceiveConnectionResponse(u8 ident, u8* data, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    DCID: {:#06x}", rsp->dcid);
  DEBUG_LOG(IOS_WIIMOTE, "    SCID: {:#06x}", rsp->scid);
  DEBUG_LOG(IOS_WIIMOTE, "    Result: {:#06x}", rsp->result);
  DEBUG_LOG(IOS_WIIMOTE, "    Status: {:#06x}", rsp->status);
...
void WiimoteDevice::ReceiveConfigurationReq(u8 ident, u8* data, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    Ident: {:#04x}", ident);
  DEBUG_LOG(IOS_WIIMOTE, "    DCID: {:#06x}", command_config_req->dcid);
  DEBUG_LOG(IOS_WIIMOTE, "    Flags: {:#06x}", command_config_req->flags);
  ...
  while (offset < size)
  {
    ...
    case L2CAP_OPT_MTU:
    {
      ...
      remote_mtu = mtu->mtu;
      DEBUG_LOG(IOS_WIIMOTE, "    MTU: {:#06x}", mtu->mtu);
    }
    break;
    ...
    case L2CAP_OPT_FLUSH_TIMO:
    {
      ...
      DEBUG_LOG(IOS_WIIMOTE, "    FlushTimeOut: {:#06x}", flush_time_out->flush_timo);
    }
    break;
...
void WiimoteDevice::ReceiveConfigurationResponse(u8 ident, u8* data, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    SCID: {:#06x}", rsp->scid);
  DEBUG_LOG(IOS_WIIMOTE, "    Flags: {:#06x}", rsp->flags);
  DEBUG_LOG(IOS_WIIMOTE, "    Result: {:#06x}", rsp->result);
...
void WiimoteDevice::ReceiveDisconnectionReq(u8 ident, u8* data, u32 size)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    Ident: {:#04x}", ident);
  DEBUG_LOG(IOS_WIIMOTE, "    DCID: {:#06x}", command_disconnection_req->dcid);
  DEBUG_LOG(IOS_WIIMOTE, "    SCID: {:#06x}", command_disconnection_req->scid);
...
void WiimoteDevice::SendConnectionRequest(u16 psm)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    Psm: {:#06x}", cr.psm);
  DEBUG_LOG(IOS_WIIMOTE, "    Scid: {:#06x}", cr.scid);
...
void WiimoteDevice::SendConfigurationRequest(u16 cid, u16 mtu, u16 flush_time_out)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    Dcid: {:#06x}", cr->dcid);
  DEBUG_LOG(IOS_WIIMOTE, "    Flags: {:#06x}", cr->flags);
  ...
  if (mtu != L2CAP_MTU_DEFAULT)
  {
    ...
    *(u16*)&buffer[offset] = mtu;
    ...
    DEBUG_LOG(IOS_WIIMOTE, "    MTU: {:#06x}", mtu);
  }
  ...
  if (flush_time_out != L2CAP_FLUSH_TIMO_DEFAULT)
  {
    ...
    *(u16*)&buffer[offset] = flush_time_out;
    ...
    DEBUG_LOG(IOS_WIIMOTE, "    FlushTimeOut: {:#06x}", flush_time_out);
  }
...
static u32 ParseCont(u8* cont)
{
  ...
  ERROR_LOG(IOS_WIIMOTE, "ParseCont: wrong cont: {}", type_id);
  PanicAlert("ParseCont: wrong cont: {}", type_id);
  return 0;
}
...
void WiimoteDevice::SendCommandToACL(u8 ident, u8 code, u8 command_length, u8* command_data)
{
  ...
  DEBUG_LOG(IOS_WIIMOTE, "    Ident: {:#04x}", ident);
  DEBUG_LOG(IOS_WIIMOTE, "    Code: {:#04x}", code);
