...
{
#if defined(WIN32)
    UDebugMessage(M64MSG_VERBOSE, "core comparison feature not supported on Windows platform.");
    return;
#else
    /* set mode */
    ...
    if (DebugSetCoreCompare(compare_core_check, compare_core_sync_data) != M64ERR_SUCCESS)
    {
        ...
        UDebugMessage(M64MSG_WARNING, "DebugSetCoreCompare() failed, core comparison disabled.");
        return;
    }
    ...
    if (l_CoreCompareMode == CORE_COMPARE_RECV)
    {
        ...
        UDebugMessage(M64MSG_INFO, "Core Comparison Waiting to read pipe.");
        fPipe = fopen("compare_pipe", "r");
    }
    else if (l_CoreCompareMode == CORE_COMPARE_SEND)
    {
        UDebugMessage(M64MSG_INFO, "Core Comparison Waiting to write pipe.");
