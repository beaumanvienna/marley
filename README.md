...
<br />
To debug marley, run "configure --enable-debug" and make in the root directory. Create the unit tests as described above. To debug PCSX2, run the unit test version ./tests/pcsx2/PCSX2 and select the interpreter instead of the recompiler under Emulation Settings / EE and IOP. Launch gdb in tests/pcsx2 and run "./PCSX2 myGame.iso". To debug dolphin, set "Fastmem" to "False" in ~/.marley/dolphin-emu/Config/Dolphin.ini.<br />
<br />
