	...
	while (pos < fmt.size()) {
		...
		if (expression.empty()) {
			...
			case 'f':
				...
				break;
			case 'p':
				snprintf(resultString, sizeof(resultString), "%08x[%08x]", expResult.u, PMemory::IsValidAddress(expResult.u) ? PMemory::Read_U32(expResult.u) : 0);
				break;
			case 's':
				snprintf(resultString, sizeof(resultString) - 1, "%s", PMemory::IsValidAddress(expResult.u) ? PMemory::GetCharPointer(expResult.u) : "(invalid)");
				break;
			case 'x':
