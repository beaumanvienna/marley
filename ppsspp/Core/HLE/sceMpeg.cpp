...
static MpegContext *getMpegCtx(u32 mpegAddr) {
	if (!PMemory::IsValidAddress(mpegAddr))
		return nullptr;
		
	u32 mpeg = PMemory::Read_U32(mpegAddr);
...
static u32 sceMpegRingbufferConstruct(u32 ringbufferAddr, u32 numPackets, u32 data, u32 size, u32 callbackAddr, u32 callbackArg) {
	if (!PMemory::IsValidAddress(ringbufferAddr)) {
...
static u32 sceMpegCreate(u32 mpegAddr, u32 dataPtr, u32 size, u32 ringbufferAddr, u32 frameWidth, u32 mode, u32 ddrTop)
{
	...
	PMemory::Write_U32(mpegHandle, mpegAddr);

	// Initialize fake mpeg struct.
	PMemory::Memcpy(mpegHandle, "LIBMPEG\0", 8);
	PMemory::Memcpy(mpegHandle + 8, "001\0", 4);
	PMemory::Write_U32(-1, mpegHandle + 12);
	if (ringbuffer.IsValid()) {
		PMemory::Write_U32(ringbufferAddr, mpegHandle + 16);
		PMemory::Write_U32(ringbuffer->dataUpperBound, mpegHandle + 20);
	}
...
static int sceMpegDelete(u32 mpeg)
{
	...
	delete ctx;
	mpegMap.erase(PMemory::Read_U32(mpeg));
...
static int sceMpegAvcDecodeMode(u32 mpeg, u32 modeAddr)
{
	...
	int mode = PMemory::Read_U32(modeAddr);
	int pixelMode = PMemory::Read_U32(modeAddr + 4);
...
{
	if (!PMemory::IsValidAddress(bufferAddr) || !PMemory::IsValidAddress(offsetAddr)) {
	...
	AnalyzeMpeg(PMemory::GetPointer(bufferAddr), PMemory::ValidSize(bufferAddr, 32768), ctx);

	if (ctx->mpegMagic != PSMF_MAGIC) {
		ERROR_LOG(ME, "sceMpegQueryStreamOffset: Bad PSMF magic");
		PMemory::Write_U32(0, offsetAddr);
	...
	PMemory::Write_U32(ctx->mpegOffset, offsetAddr);
	return 0;
}

static u32 sceMpegQueryStreamSize(u32 bufferAddr, u32 sizeAddr)
{
	if (!PMemory::IsValidAddress(bufferAddr) || !PMemory::IsValidAddress(sizeAddr)) {
	...
	PMemory::Write_U32(ctx.mpegStreamSize, sizeAddr);
	return 0;
}
...
static bool decodePmpVideo(PSPPointer<SceMpegRingBuffer> ringbuffer, u32 pmpctxAddr){
	...
	if (PMemory::IsValidAddress(pmp_videoSource)){
		...
		for (int i = 0; i < pmp_nBlocks; i++){
			PMemory::ReadStructUnchecked(pmp_videoSource, &lli);
			// add source block into pmpframes
			pmpframes->add(PMemory::GetPointer(lli.pSrc), lli.iSize);
			// get next block
...
static u32 sceMpegAvcDecode(u32 mpeg, u32 auAddr, u32 frameWidth, u32 bufferAddr, u32 initAddr)
{
	...
	u32 buffer = PMemory::Read_U32(bufferAddr);
	u32 init = PMemory::Read_U32(initAddr);
	...
	PMemory::Write_U32(ctx->avc.avcFrameStatus, initAddr);
	ctx->avc.avcDecodeResult = MPEG_AVC_DECODE_SUCCESS;
...
{
	if (!PMemory::IsValidAddress(bufferAddr) || !PMemory::IsValidAddress(statusAddr)){
...
{
	if (!PMemory::IsValidAddress(detailAddr)){
	...
	PMemory::Write_U32(ctx->avc.avcDecodeResult, detailAddr + 0);
	PMemory::Write_U32(ctx->videoFrameCount, detailAddr + 4);
	PMemory::Write_U32(ctx->avc.avcDetailFrameWidth, detailAddr + 8);
	PMemory::Write_U32(ctx->avc.avcDetailFrameHeight, detailAddr + 12);
	PMemory::Write_U32(0, detailAddr + 16);
	PMemory::Write_U32(0, detailAddr + 20);
	PMemory::Write_U32(0, detailAddr + 24);
	PMemory::Write_U32(0, detailAddr + 28);
	PMemory::Write_U32(ctx->avc.avcFrameStatus, detailAddr + 32);
	return 0;
}
...
{
	if (!PMemory::IsValidAddress(esSizeAddr) || !PMemory::IsValidAddress(outSizeAddr)) {
	...
	PMemory::Write_U32(MPEG_ATRAC_ES_SIZE, esSizeAddr);
	PMemory::Write_U32(MPEG_ATRAC_ES_OUTPUT_SIZE, outSizeAddr);
	return 0;
}
...
void PostPutAction::run(MipsCall &call) {
	...
	const u8 *data = PMemory::GetPointer(ringbuffer->data + writeOffset * 2048);
	...
	if (mpegLibVersion < 0x0105 && packetsAdded > 0) {
		...
		const u8 *buf = PMemory::GetPointer(ringbuffer->data + readOffset * 2048);
		bool invalid = false;
...
static int sceMpegGetAvcAu(u32 mpeg, u32 streamId, u32 auAddr, u32 attrAddr)
{
	...
	if (PMemory::IsValidAddress(attrAddr)) {
		PMemory::Write_U32(1, attrAddr);
	}
...
static int sceMpegQueryPcmEsSize(u32 mpeg, u32 esSizeAddr, u32 outSizeAddr)
{
	...
	PMemory::Write_U32(MPEG_PCM_ES_SIZE, esSizeAddr);
	PMemory::Write_U32(MPEG_PCM_ES_OUTPUT_SIZE, outSizeAddr);
	return 0;
}
...
{
	if (!PMemory::IsValidAddress(stream_addr)) {
...
static u32 sceMpegGetPcmAu(u32 mpeg, int streamUid, u32 auAddr, u32 attrAddr)
{
	...
	if (!PMemory::IsValidAddress(streamUid)) {
	...
	if (PMemory::IsValidAddress(attrAddr))
		PMemory::Write_U32(attr, attrAddr);
...
{
	if (!PMemory::IsValidAddress(sourceAddr) || !PMemory::IsValidAddress(YCbCrAddr)) {
...
static u32 sceMpegAtracDecode(u32 mpeg, u32 auAddr, u32 bufferAddr, int init)
{
	...
	if (!PMemory::IsValidAddress(bufferAddr)) {
	...
	PMemory::Memset(bufferAddr, 0, MPEG_ATRAC_ES_OUTPUT_SIZE);
...
{
	if (!PMemory::IsValidAddress(sourceAddr) || !PMemory::IsValidAddress(rangeAddr) || !PMemory::IsValidAddress(destAddr)) {
	...
	int x = PMemory::Read_U32(rangeAddr);
	int y = PMemory::Read_U32(rangeAddr + 4);
	int width = PMemory::Read_U32(rangeAddr + 8);
	int height = PMemory::Read_U32(rangeAddr + 12);
...
{
	if (!PMemory::IsValidAddress(ycbcr_addr)) {
...
static int sceMpegAvcQueryYCbCrSize(u32 mpeg, u32 mode, u32 width, u32 height, u32 resultAddr)
{
	...
	PMemory::Write_U32(size, resultAddr);
	return 0;
}
...
static u32 sceMpegQueryUserdataEsSize(u32 mpeg, u32 esSizeAddr, u32 outSizeAddr)
{
	...
	PMemory::Write_U32(MPEG_DATA_ES_SIZE, esSizeAddr);
	PMemory::Write_U32(MPEG_DATA_ES_OUTPUT_SIZE, outSizeAddr);
	return 0;
}
...
static int __MpegAvcConvertToYuv420(const void *data, u32 bufferOutputAddr, int width, int height) {
	...
	int sizeCb = sizeY >> 2;
	u8 *Y = (u8*)PMemory::GetPointer(bufferOutputAddr);
	u8 *Cb = Y + sizeY;
	u8 *Cr = Cb + sizeCb;
...
{
	if (!PMemory::IsValidAddress(bufferOutputAddr)) {
...
static int sceMpegGetUserdataAu(u32 mpeg, u32 streamUid, u32 auAddr, u32 resultAddr)
{
	...
	PMemory::Write_U32(0, resultAddr);
	PMemory::Write_U32(0, resultAddr + 4);
