		...
		if (waitingThread.threadID == threadID) {
			...
			if (Microphone::isHaveDevice()) {
				if (Microphone::availableAudioBufSize() >= waitingThread.needSize) {
					...
					PMemory::Memcpy(waitingThread.addr, tempbuf8, waitingThread.needSize);
					delete[] tempbuf8;
					...
					PCoreTiming::ScheduleEvent(usToCycles(waitTimeus), eventUsbMicAudioUpdate, userdata);
				}
			} else {
				for (u32 i = 0; i < waitingThread.needSize; i++) {
					if (PMemory::IsValidAddress(waitingThread.addr + i)) {
						PMemory::Write_U8(i & 0xFF, waitingThread.addr + i);
					}
				}
				u32 ret = __KernelGetWaitValue(threadID, error);
				DEBUG_LOG(HLE, "sceUsbMic: Waking up thread(%d)", (int)waitingThread.threadID);
...
void __UsbMicInit() {
	...
	curChannels = 1;
	micState = 0; 
	eventUsbMicAudioUpdate = PCoreTiming::RegisterEvent("UsbMicAudioUpdate", &__UsbMicAudioUpdate);
}
...
void __UsbMicDoState(PointerWrap &p) {
	...
	if (waitingThreads.size() != 0 && p.mode == PointerWrap::MODE_READ) {
		...
		PCoreTiming::ScheduleEvent(usToCycles(waitTimeus), eventUsbMicAudioUpdate, waitingThreads[0].threadID);
	}
	if (micState == 0) {
...
u32 __MicInputBlocking(u32 maxSamples, u32 sampleRate, u32 bufAddr) {
	...
	PCoreTiming::ScheduleEvent(usToCycles(waitTimeus), eventUsbMicAudioUpdate, __KernelGetCurThread());
