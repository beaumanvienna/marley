...
void InitLocalhostIP() {
	...
	std::string serverStr = StripSpaces(g_PConfig.proAdhocServer);
...
int ScheduleApctlState(int event, int newState, int usec, const char* reason) {
	...
	if (apctlStateEvent < 0)
		apctlStateEvent = PCoreTiming::RegisterEvent("__ApctlState", __ApctlState);

	u64 param = ((u64)__KernelGetCurThread()) << 32 | uid;
	PCoreTiming::ScheduleEvent(usToCycles(usec), apctlStateEvent, param);
...
void netValidateLoopMemory() {
	...
	if (!apctlThreadHackAddr || (apctlThreadHackAddr && strcmp("apctlThreadHack", kernelMemory.GetBlockTag(apctlThreadHackAddr)) != 0)) {
		...
		if (apctlThreadHackAddr) PMemory::Memcpy(apctlThreadHackAddr, apctlThreadCode, sizeof(apctlThreadCode));
	}
}
...
void __NetDoState(PointerWrap &p) {
	...
	if (s >= 5) {
		...
		if (apctlStateEvent != -1) {
			PCoreTiming::RestoreRegisterEvent(apctlStateEvent, "__ApctlState", __ApctlState);
		}
	}
	else {
		apctlStateEvent = -1;
...
static u32 sceWlanGetEtherAddr(u32 addrAddr) {
	if (!PMemory::IsValidRange(addrAddr, 6)) {
		// More correctly, it should crash.
		return hleLogError(SCENET, SCE_KERNEL_ERROR_ILLEGAL_ADDR, "illegal address");
	}

	u8 *addr = PMemory::GetPointer(addrAddr);
	if (PPSSPP_ID > 1) {
		PMemory::Memset(addrAddr, PPSSPP_ID, 6);
	}
	else
...
static void sceNetEtherNtostr(u32 macPtr, u32 bufferPtr) {
	...
	if (PMemory::IsValidAddress(bufferPtr) && PMemory::IsValidAddress(macPtr)) {
		char *buffer = (char *)PMemory::GetPointer(bufferPtr);
		const u8 *mac = PMemory::GetPointer(macPtr);
...
static void sceNetEtherStrton(u32 bufferPtr, u32 macPtr) {
	...
	if (PMemory::IsValidAddress(bufferPtr) && PMemory::IsValidAddress(macPtr)) {
		const char *buffer = (char *)PMemory::GetPointer(bufferPtr);
		u8 *mac = PMemory::GetPointer(macPtr);
		...
		VERBOSE_LOG(SCENET, "sceNetEtherStrton - [%s]", mac2str((SceNetEtherAddr*)PMemory::GetPointer(macPtr)).c_str());
...
static int sceNetGetMallocStat(u32 statPtr) {
	...
	if(PMemory::IsValidAddress(statPtr))
		PMemory::WriteStruct(statPtr, &netMallocStat);
	else
...
void NetApctl_InitInfo() {
	...
	netApctlInfo.channel = g_PConfig.iWlanAdhocChannel;
...
static int sceNetApctlGetState(u32 pStateAddr) {
	...
	// Valid Arguments
	if (PMemory::IsValidAddress(pStateAddr)) {
		// Return Thread Status
		PMemory::Write_U32(NetApctl_GetState(), pStateAddr);
		// Return Success
		return hleLogSuccessI(SCENET, 0);
	}
...
static int sceNetApctlGetBSSDescIDListUser(u32 sizeAddr, u32 bufAddr) {
	...
	int size = PMemory::Read_U32(sizeAddr);
	// Return size required
	PMemory::Write_U32(entries * userInfoSize, sizeAddr);

	if (bufAddr != 0 && PMemory::IsValidAddress(sizeAddr)) {
		int offset = 0;
		for (int i = 0; i < entries; i++) {
			...
			PMemory::Write_U32((i+1)*userInfoSize + bufAddr, bufAddr + offset);
			offset += 4;

			// Entry ID
			PMemory::Write_U32(i, bufAddr + offset);
			offset += 4;
		}
		// Fix the last Pointer
		if (offset > 0)
			PMemory::Write_U32(0, bufAddr + offset - userInfoSize);
	}
...
static int sceNetApctlGetBSSDescEntryUser(int entryId, int infoId, u32 resultAddr) {
	...
	if (!PMemory::IsValidAddress(resultAddr))
		hleLogError(SCENET, -1, "apctl invalid arg");

	switch (infoId) {
	case PSP_NET_APCTL_DESC_IBSS: // IBSS, 6 bytes
		PMemory::WriteStruct(resultAddr, &netApctlInfo.bssid);
		break;
	case PSP_NET_APCTL_DESC_SSID_NAME:
		// Return 32 bytes
		PMemory::WriteStruct(resultAddr, &netApctlInfo.ssid);
		break;
	case PSP_NET_APCTL_DESC_SSID_NAME_LENGTH:
		// Return one 32-bit value
		PMemory::WriteStruct(resultAddr, &netApctlInfo.ssidLength);
		break;
	case PSP_NET_APCTL_DESC_SIGNAL_STRENGTH:
		// Return 1 byte
		PMemory::WriteStruct(resultAddr, &netApctlInfo.strength);
		break;
	case PSP_NET_APCTL_DESC_SECURITY:
		// Return one 32-bit value
		PMemory::WriteStruct(resultAddr, &netApctlInfo.securityType);
		break;
	default:
...
{
	PMemory::Write_U32(netDropRate, dropRateAddr);
	PMemory::Write_U32(netDropDuration, dropDurationAddr);
