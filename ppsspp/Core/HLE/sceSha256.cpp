...
static int sceSha256Digest(u32 data, int dataLen, u32 digestPtr) {
	if (!PMemory::IsValidAddress(data) || !PMemory::IsValidAddress(digestPtr) || !PMemory::IsValidAddress(data + dataLen)) {
	...
	u8 *digest = PMemory::GetPointerUnchecked(digestPtr);
	sha256_context ctx;
	sha256_starts(&ctx);
	sha256_update(&ctx, PMemory::GetPointerUnchecked(data), dataLen);
