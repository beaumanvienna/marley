...
static int Replace_memcpy() {
	...
	if ((skipGPUReplacements & (int)GPUReplacementSkip::MEMCPY) == 0) {
		if (Memory_P::IsVRAMAddress(destPtr) || Memory_P::IsVRAMAddress(srcPtr)) {
			skip = gpu->PerformMemoryCopy(destPtr, srcPtr, bytes);
		}
	}
	if (!skip && bytes != 0) {
		u8 *dst = Memory_P::GetPointer(destPtr);
		const u8 *src = Memory_P::GetPointer(srcPtr);
...
static int Replace_memcpy_swizzled() {
	...
	u32 h = PARAM(4);
	if ((skipGPUReplacements & (int)GPUReplacementSkip::MEMCPY) == 0) {
		if (Memory_P::IsVRAMAddress(srcPtr)) {
			gpu->PerformMemoryDownload(srcPtr, pitch * h);
		}
	}
	u8 *dstp = Memory_P::GetPointer(destPtr);
	const u8 *srcp = Memory_P::GetPointer(srcPtr);
...
static int Replace_memset() {
	...
	u32 bytes = PARAM(2);
	bool skip = false;
	if (Memory_P::IsVRAMAddress(destPtr) && (skipGPUReplacements & (int)GPUReplacementSkip::MEMSET) == 0) {
...
static int Replace_strlen() {
	u32 srcPtr = PARAM(0);
	const char *src = (const char *)Memory_P::GetPointer(srcPtr);
...
static int Replace_strcpy() {
	u32 destPtr = PARAM(0);
	char *dst = (char *)Memory_P::GetPointer(destPtr);
	const char *src = (const char *)Memory_P::GetPointer(PARAM(1));
	if (dst && src) {
		strcpy(dst, src);
	}
	RETURN(destPtr);
...
static int Replace_strcmp() {
	const char *a = (const char *)Memory_P::GetPointer(PARAM(0));
	const char *b = (const char *)Memory_P::GetPointer(PARAM(1));
	if (a && b) {
		RETURN(strcmp(a, b));
	} else {
		RETURN(0);
...
static int Replace_vmmul_q_transp() {
	float *out = (float *)Memory_P::GetPointer(PARAM(0));
	const float *a = (const float *)Memory_P::GetPointer(PARAM(1));
	const float *b = (const float *)Memory_P::GetPointer(PARAM(2));
...
static int Replace_gta_dl_write_matrix() {
	u32 *ptr = (u32 *)Memory_P::GetPointer(PARAM(0));
	u32 *src = (u32_le *)Memory_P::GetPointer(PARAM(2));
	...
	u32 *dest = (u32_le *)Memory_P::GetPointer(ptr[0]);
	if (!dest) {
		RETURN(0);
		return 38;
	}
...
static int Replace_dl_write_matrix() {
	u32 *dlStruct = (u32 *)Memory_P::GetPointer(PARAM(0));
	u32 *src = (u32 *)Memory_P::GetPointer(PARAM(2));
	...
	u32 *dest = (u32 *)Memory_P::GetPointer(dlStruct[2]);
	if (!dest) {
		RETURN(0);
		return 60;
	}
...
static bool GetMIPSStaticAddress(u32 &addr, s32 lui_offset, s32 lw_offset) {
	const MIPSOpcode upper = Memory_P::Read_Instruction(currentMIPS->pc + lui_offset, true);
	if (upper != MIPS_MAKE_LUI(MIPS_GET_RT(upper), upper & 0xffff)) {
		return false;
	}
	const MIPSOpcode lower = Memory_P::Read_Instruction(currentMIPS->pc + lw_offset, true);
...
static bool GetMIPSGPAddress(u32 &addr, s32 offset) {
	const MIPSOpcode loadOp = Memory_P::Read_Instruction(currentMIPS->pc + offset, true);
...
static int Hook_godseaterburst_blit_texture() {
	...
	u32 fb_infoaddr;
	if (Memory_P::PRead_U32(texaddr) != 0 || !GetMIPSStaticAddress(fb_infoaddr, 0x01d0, 0x01d4)) {
		return 0;
	}

	const u32 fb_info = Memory_P::PRead_U32(fb_infoaddr);
	const u32 fb_address = Memory_P::PRead_U32(fb_info);
	if (Memory_P::IsVRAMAddress(fb_address)) {
...
static int Hook_brandish_download_frame() {
	...
	const u32 fb_info = Memory_P::PRead_U32(fb_infoaddr);
	const MIPSOpcode fb_index_load = Memory_P::Read_Instruction(currentMIPS->pc + 0x38, true);
	...
	const u32 fb_index = (Memory_P::PRead_U32(fb_info + fb_index_offset) + 1) & 1;
	const u32 fb_address = 0x4000000 + (0x44000 * fb_index);
	const u32 dest_address = currentMIPS->r[MIPS_REG_A1];
	if (Memory_P::IsRAMAddress(dest_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00044000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00044000, currentMIPS->pc);
	}
	return 0;
...
static int Hook_growlanser_create_saveicon() {
	const u32 fb_address = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_SP] + 4);
	const u32 fmt = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_SP]);
	const u32 sz = fmt == GE_FORMAT_8888 ? 0x00088000 : 0x00044000;
	if (Memory_P::IsVRAMAddress(fb_address) && fmt <= 3) {
...
static int Hook_sd_gundam_g_generation_download_frame() {
	const u32 fb_address = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_SP] + 8);
	const u32 fmt = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_SP] + 4);
	const u32 sz = fmt == GE_FORMAT_8888 ? 0x00088000 : 0x00044000;
	if (Memory_P::IsVRAMAddress(fb_address) && fmt <= 3) {
		gpu->PerformMemoryDownload(fb_address, sz);
		CBreakPoints::ExecMemCheck(fb_address, true, sz, currentMIPS->pc);
...
static int Hook_rezel_cross_download_frame() {
	const u32 fb_address = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_SP] + 0x1C);
	const u32 fmt = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_SP] + 0x14);
	const u32 sz = fmt == GE_FORMAT_8888 ? 0x00088000 : 0x00044000;
	if (Memory_P::IsVRAMAddress(fb_address) && fmt <= 3) {
		gpu->PerformMemoryDownload(fb_address, sz);
		CBreakPoints::ExecMemCheck(fb_address, true, sz, currentMIPS->pc);
...
static int Hook_soranokiseki_sc_download_frame() {
	...
	const u32 fb_info = Memory_P::PRead_U32(fb_infoaddr);
	const MIPSOpcode fb_index_load = Memory_P::Read_Instruction(currentMIPS->pc + 0x34, true);
	if (fb_index_load != MIPS_MAKE_LW(MIPS_GET_RT(fb_index_load), MIPS_GET_RS(fb_index_load), fb_index_load & 0xffff)) {
		return 0;
	}
	const int fb_index_offset = (s16)(fb_index_load & 0xffff);
...
static int Hook_danganronpa1_2_download_frame() {
	const MIPSOpcode instruction = Memory_P::Read_Instruction(currentMIPS->pc + 0x8, true);
...
static int Hook_motorstorm_download_frame() {
	const u32 fb_address = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_A1] + 0x18);
	if (Memory_P::IsVRAMAddress(fb_address)) {
		gpu->PerformMemoryDownload(fb_address, 0x00088000);
		CBreakPoints::ExecMemCheck(fb_address, true, 0x00088000, currentMIPS->pc);
	}
...
static int Hook_katamari_render_check() {
	const u32 fb_address = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_A0] + 0x3C);
	const u32 fbInfoPtr = Memory_P::PRead_U32(currentMIPS->r[MIPS_REG_A0] + 0x40);
	if (Memory_P::IsVRAMAddress(fb_address) && fbInfoPtr != 0) {
		const u32 sizeInfoPtr = Memory_P::PRead_U32(fbInfoPtr + 0x0C);
		// These are the values it uses to control the loop.
		// Width in memory appears to be stride / 8.
		const u32 width = Memory_P::PRead_U16(sizeInfoPtr + 0x08) * 8;
		// Height in memory is also divided by 8 (but this one isn't hardcoded.)
		const u32 heightBlocks = Memory_P::PRead_U16(sizeInfoPtr + 0x0A);
		// For some reason this is the number of heightBlocks less 1.
		const u32 heightBlockCount = Memory_P::PRead_U8(fbInfoPtr + 0x08) + 1;
...
static int Hook_starocean_clear_framebuf_after() {
	...
	if (GetMIPSGPAddress(y_address, -204) && GetMIPSGPAddress(h_address, -200)) {
		int y = (s16)Memory_P::PRead_U16(y_address);
		int h = (s16)Memory_P::PRead_U16(h_address);
...
static bool WriteReplaceInstruction(u32 address, int index) {
	u32 prevInstr = Memory_P::Read_Instruction(address, false).encoding;
	...
	if (MIPS_IS_RUNBLOCK(Memory_P::PRead_U32(address))) {
		WARN_LOG(HLE, "Replacing jitted func address %08x", address);
	}
	replacedInstructions[address] = prevInstr;
	Memory_P::PWrite_U32(MIPS_EMUHACK_CALL_REPLACEMENT | index, address);
	return true;
}
	...
	for (int index : indexes) {
		...
		if (entry->flags & REPFLAG_HOOKEXIT) {
			...
			for (u32 offset = 0; offset < (u32)size; offset += 4) {
				const u32 op = Memory_P::Read_Instruction(address + offset, false).encoding;
...
void RestoreReplacedInstruction(u32 address) {
	const u32 curInstr = Memory_P::PRead_U32(address);
	if (MIPS_IS_REPLACEMENT(curInstr)) {
		Memory_P::PWrite_U32(replacedInstructions[address], address);
...
void RestoreReplacedInstructions(u32 startAddr, u32 endAddr) {
	...
	for (auto it = start; it != end; ++it) {
		...
		const u32 curInstr = Memory_P::PRead_U32(addr);
		if (MIPS_IS_REPLACEMENT(curInstr)) {
			Memory_P::PWrite_U32(it->second, addr);
			++restored;
		}
	}
...
bool GetReplacedOpAt(u32 address, u32 *op) {
	u32 instr = Memory_P::Read_Opcode_JIT(address).encoding;
...
bool CanReplaceJalTo(u32 dest, const ReplacementTableEntry **entry, u32 *funcSize) {
	MIPSOpcode op(Memory_P::Read_Opcode_JIT(dest));
