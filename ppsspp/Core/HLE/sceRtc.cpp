...
static u64 __RtcGetCurrentTick()
{
	...
	return PCoreTiming::GetGlobalTimeUs() + rtcBaseTicks;
}
...
{
	s64 additionalUs = PCoreTiming::GetGlobalTimeUs();
	*tv = rtcBaseTime;
...
static u32 sceRtcGetCurrentTick(u32 tickPtr)
{
	...
	if (PMemory::IsValidAddress(tickPtr))
		PMemory::Write_U64(curTick, tickPtr);
	hleEatCycles(300);
...
static u32 sceRtcGetCurrentClock(u32 pspTimePtr, int tz)
{
	...
	if (PMemory::IsValidAddress(pspTimePtr))
		PMemory::WriteStruct(pspTimePtr, &ret);
...
static u32 sceRtcGetCurrentClockLocalTime(u32 pspTimePtr)
{
	...
	if (PMemory::IsValidAddress(pspTimePtr))
		PMemory::WriteStructUnchecked(pspTimePtr, &ret);
...
static u32 sceRtcSetTick(u32 pspTimePtr, u32 tickPtr)
{
	...
	if (PMemory::IsValidAddress(pspTimePtr) && PMemory::IsValidAddress(tickPtr))
	{
		u64 ticks = PMemory::Read_U64(tickPtr);
...
static u32 sceRtcGetTick(u32 pspTimePtr, u32 tickPtr)
{
	...
	if (PMemory::IsValidAddress(pspTimePtr) && PMemory::IsValidAddress(tickPtr))
	{
		PMemory::ReadStructUnchecked(pspTimePtr, &pt);
		...
		PMemory::Write_U64(result, tickPtr);
	}
...
static int sceRtcConvertLocalTimeToUTC(u32 tickLocalPtr,u32 tickUTCPtr)
{
	...
	if (PMemory::IsValidAddress(tickLocalPtr) && PMemory::IsValidAddress(tickUTCPtr))
	{
		u64 srcTick = PMemory::Read_U64(tickLocalPtr);
...
#endif
		PMemory::Write_U64(srcTick, tickUTCPtr);
	}
	else
	{
		return 1;
...
static int sceRtcConvertUtcToLocalTime(u32 tickUTCPtr,u32 tickLocalPtr)
{
	...
	if (PMemory::IsValidAddress(tickLocalPtr) && PMemory::IsValidAddress(tickUTCPtr))
	{
		u64 srcTick = PMemory::Read_U64(tickUTCPtr);
		// TODO : Let the user select his timezone / daylight saving instead of taking system param ?
#ifdef _WIN32
		long timezone_val;
		_get_timezone(&timezone_val);
		...
		srcTick += time->tm_gmtoff*1000000ULL;
#endif
		PMemory::Write_U64(srcTick, tickLocalPtr);
	}
	else
	{
		return 1;
...
static int sceRtcCheckValid(u32 datePtr)
{
	...
	if (PMemory::IsValidAddress(datePtr))
	{
		ScePspDateTime pt;
		PMemory::ReadStructUnchecked(datePtr, &pt);
...
static int sceRtcSetTime_t(u32 datePtr, u32 time)
{
	...
	if (PMemory::IsValidAddress(datePtr))
	{
		...
		PMemory::WriteStructUnchecked(datePtr, &pt);
	}
	else
	{
		return 1;
...
static int sceRtcSetTime64_t(u32 datePtr, u64 time)
{
	...
	if (PMemory::IsValidAddress(datePtr))
	{
		ScePspDateTime pt;
		__RtcTicksToPspTime(pt, time*1000000ULL + rtcMagicOffset);
		PMemory::WriteStruct(datePtr, &pt);
	}
	else
	{
		return 1;
...
static int sceRtcGetTime_t(u32 datePtr, u32 timePtr)
{
	...
	if (PMemory::IsValidAddress(datePtr)&&PMemory::IsValidAddress(timePtr))
	{
		ScePspDateTime pt;
		PMemory::ReadStructUnchecked(datePtr, &pt);
		u32 result = (u32) ((__RtcPspTimeToTicks(pt)-rtcMagicOffset)/1000000ULL);
		PMemory::Write_U32(result, timePtr);
	}
	else
	{
		return 1;
...
static int sceRtcGetTime64_t(u32 datePtr, u32 timePtr)
{
	...
	if (PMemory::IsValidAddress(datePtr)&&PMemory::IsValidAddress(timePtr))
	{
		...
		PMemory::Write_U64(result, timePtr);
	}
	else
	{
		return 1;
...
static int sceRtcGetDosTime(u32 datePtr, u32 dosTime)
{
	...
	if (PMemory::IsValidAddress(datePtr)&&PMemory::IsValidAddress(dosTime))
	{
		ScePspDateTime pt;
		PMemory::ReadStructUnchecked(datePtr, &pt);
		...
		PMemory::Write_U32(result, dosTime);
	}
	else
	{
		retValue = -1;
...
static int sceRtcGetWin32FileTime(u32 datePtr, u32 win32TimePtr)
{
	...
	if (!PMemory::IsValidAddress(win32TimePtr))
		return SCE_KERNEL_ERROR_INVALID_VALUE;
	...
	{
		PMemory::Write_U64(0, win32TimePtr);
		return SCE_KERNEL_ERROR_INVALID_VALUE;
	}

	PMemory::Write_U64((result - rtcFiletimeOffset) * 10, win32TimePtr);
	return 0;
}
...
static int sceRtcCompareTick(u32 tick1Ptr, u32 tick2Ptr)
{
	...
	if (PMemory::IsValidAddress(tick1Ptr) && PMemory::IsValidAddress(tick2Ptr))
	{
		u64 tick1 = PMemory::Read_U64(tick1Ptr);
		u64 tick2 = PMemory::Read_U64(tick2Ptr);
		if (tick1 > tick2)
			return 1;
		if (tick1 < tick2)
			return -1;
...
{
	if (PMemory::IsValidAddress(destTickPtr) && PMemory::IsValidAddress(srcTickPtr))
	{
		u64 srcTick = PMemory::Read_U64(srcTickPtr);

		srcTick += numTicks;
		PMemory::Write_U64(srcTick, destTickPtr);
	}
...
static int sceRtcTickAddMicroseconds(u32 destTickPtr,u32 srcTickPtr, u64 numMS)
{
	if (PMemory::IsValidAddress(destTickPtr) && PMemory::IsValidAddress(srcTickPtr))
	{
		s64 srcTick = (s64)PMemory::Read_U64(srcTickPtr);
...
static int sceRtcTickAddMonths(u32 destTickPtr, u32 srcTickPtr, int numMonths)
{
	...
	if (__RtcValidatePspTime(pt))
	{
		...
		if (pt.month == 2 && pt.day == 29 && !__RtcIsLeapYear((s16)pt.year))
			pt.day = 28;
		PMemory::Write_U64(__RtcPspTimeToTicks(pt), destTickPtr);
	}
...
{
	if (PMemory::IsValidAddress(tickPtr))
		PMemory::Write_U64(rtcLastAdjustedTicks, tickPtr);
...
{
	if (PMemory::IsValidAddress(tickPtr))
		PMemory::Write_U64(rtcLastReincarnatedTicks, tickPtr);
...
static int __RtcFormatRFC2822(u32 outPtr, u32 srcTickPtr, int tz)
{
	...
	char *out = (char *)PMemory::GetPointer(outPtr);
	char *end = out + 32;
...
{
	if (!PMemory::IsValidAddress(outPtr) || !PMemory::IsValidAddress(srcTickPtr))
	{
