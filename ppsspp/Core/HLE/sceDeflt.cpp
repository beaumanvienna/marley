...
static int sceDeflateDecompress(u32 OutBuffer, int OutBufferLength, u32 InBuffer, u32 Crc32Addr) {
	...
	if (!PMemory::IsValidAddress(OutBuffer) || !PMemory::IsValidAddress(InBuffer)) {
		ERROR_LOG(HLE, "sceZlibDecompress: Bad address %08x %08x", OutBuffer, InBuffer);
		return 0;
	}
	if (Crc32Addr) {
		if (!PMemory::IsValidAddress(Crc32Addr)) {
			ERROR_LOG(HLE, "sceZlibDecompress: Bad address %08x", Crc32Addr);
			return 0;
		}
		crc32AddrPtr = (u32 *)PMemory::GetPointer(Crc32Addr);
	}
	outBufferPtr = PMemory::GetPointer(OutBuffer);
	stream.next_in = (Bytef*)PMemory::GetPointer(InBuffer);
