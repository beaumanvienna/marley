...
static PluginInfo ReadPluginIni(const std::string &subdir, PIniFile &ini) {
	PluginInfo info;
...
static std::vector<PluginInfo> FindPlugins(const std::string &gameID, const std::string &lang) {
	...
	for (auto subdir : pluginDirs) {
		if (!subdir.isDirectory || !PFile::Exists(subdir.fullName + "/plugin.ini"))
			continue;
...
void Init() {
	if (!g_PConfig.bLoadPlugins) {
		return;
	}

	std::vector<PluginInfo> plugins = FindPlugins(g_paramSFO.GetDiscID(), g_PConfig.sLanguageIni);
	for (auto &plugin : plugins) {
		if (plugin.memory << 20 > PMemory::g_MemorySize) {
			PMemory::g_MemorySize = plugin.memory << 20;
			anyEnabled = true;
		}
