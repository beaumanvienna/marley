...
void __PsmfInit() {
	...
	eventPsmfPlayerStatusChange = PCoreTiming::RegisterEvent("PsmfPlayerStatusChange", &__PsmfPlayerStatusChange);
}
...
void __PsmfPlayerDoState(PointerWrap &p) {
	...
	if (s < 3) {
		...
		PCoreTiming::RestoreRegisterEvent(eventPsmfPlayerStatusChange, "PsmfPlayerStatusChangeEvent", &__PsmfPlayerStatusChange);
	}
	if (s < 2) {
...
static void DelayPsmfStateChange(u32 psmfPlayer, u32 newState, s64 delayUs) {
	...
	PCoreTiming::ScheduleEvent(usToCycles(delayUs), eventPsmfPlayerStatusChange, (u64)psmfPlayer << 32 | newState);
}
