...
void handleTimeout(SceNetAdhocMatchingContext * context)
{
	...
	while (peer != NULL && contexts != NULL && coreState != CORE_POWERDOWN)
	{
		...
		u64_le now = PCoreTiming::GetGlobalTimeUsScaled(); //time_now_d()*1000000.0
...
int friendFinder(){
	...
	if (!net::DNSResolve(g_PConfig.proAdhocServer, "", &resolved, err)) {
		ERROR_LOG(SCENET, "DNS Error Resolving %s\n", g_PConfig.proAdhocServer.c_str());
		host->NotifyUserMessage(n->T("DNS Error Resolving ") + g_PConfig.proAdhocServer, 2.0f, 0x0000ff);
	}
	if (resolved) {
	...
	while (friendFinderRunning) {
		...
		if (networkInited) {
			// Ping Server
			now = time_now_d() * 1000000.0; // Use time_now_d()*1000000.0 instead of PCoreTiming::GetGlobalTimeUsScaled() if the game gets disconnected from AdhocServer too soon when FPS wasn't stable
...
int initNetwork(SceNetAdhocctlAdhocId *adhoc_id){
	...
	if (g_adhocServerIP.in.sin_addr.s_addr == g_localhostIP.in.sin_addr.s_addr && !g_PConfig.bEnableAdhocServer)
		return -1;
	...
	if (iResult == SOCKET_ERROR && errorcode != EISCONN) {
		...
		if (IsSocketReady(metasocket, false, true) <= 0) {
			ERROR_LOG(SCENET, "Socket error (%i) when connecting to AdhocServer [%s/%s:%u]", errorcode, g_PConfig.proAdhocServer.c_str(), inet_ntoa(g_adhocServerIP.in.sin_addr), ntohs(g_adhocServerIP.in.sin_port));
