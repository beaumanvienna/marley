...
{
	mutexWaitTimer = PCoreTiming::RegisterEvent("MutexTimeout", __KernelMutexTimeout);
	lwMutexWaitTimer = PCoreTiming::RegisterEvent("LwMutexTimeout", __KernelLwMutexTimeout);
...
void __KernelMutexDoState(PointerWrap &p)
{
	...
	Do(p, mutexWaitTimer);
	PCoreTiming::RestoreRegisterEvent(mutexWaitTimer, "MutexTimeout", __KernelMutexTimeout);
	Do(p, lwMutexWaitTimer);
	PCoreTiming::RestoreRegisterEvent(lwMutexWaitTimer, "LwMutexTimeout", __KernelLwMutexTimeout);
	Do(p, mutexHeldLocks);
}
...
static bool __KernelUnlockMutexForThread(PSPMutex *mutex, SceUID threadID, u32 &error, int result) {
	...
	if (timeoutPtr != 0 && mutexWaitTimer != -1)
	{
		...
		s64 cyclesLeft = PCoreTiming::UnscheduleEvent(mutexWaitTimer, threadID);
...
static void __KernelWaitMutex(PSPMutex *mutex, u32 timeoutPtr) {
	...
	PCoreTiming::ScheduleEvent(usToCycles(micro), mutexWaitTimer, __KernelGetCurThread());
}
...
int sceKernelCancelMutex(SceUID uid, int count, u32 numWaitThreadsPtr)
{
	...
	if (mutex)
	{
		...
		if (PMemory::IsValidAddress(numWaitThreadsPtr))
			PMemory::Write_U32((u32)mutex->waitingThreads.size(), numWaitThreadsPtr);
...
int sceKernelReferMutexStatus(SceUID id, u32 infoAddr)
{
	...
	if (PMemory::Read_U32(infoAddr) != 0)
	{
		...
		PMemory::WriteStruct(infoAddr, &m->nm);
	}
	return 0;
}
...
bool __KernelUnlockLwMutexForThread(LwMutex *mutex, T workarea, SceUID threadID, u32 &error, int result)
{
	...
	{
		// Remove any event for this thread.
		s64 cyclesLeft = PCoreTiming::UnscheduleEvent(lwMutexWaitTimer, threadID);
		PMemory::Write_U32((u32) cyclesToUs(cyclesLeft), timeoutPtr);
	}
...
int sceKernelDeleteLwMutex(u32 workareaPtr)
{
	...
	if (!workareaPtr || !PMemory::IsValidAddress(workareaPtr))
...
static void __KernelWaitLwMutex(LwMutex *mutex, u32 timeoutPtr)
{
	...
	PCoreTiming::ScheduleEvent(usToCycles(micro), lwMutexWaitTimer, __KernelGetCurThread());
}
...
int sceKernelTryLockLwMutex(u32 workareaPtr, int count)
{
	...
	if (!PMemory::IsValidAddress(workareaPtr)) {
...
static int __KernelReferLwMutexStatus(SceUID uid, u32 infoPtr)
{
	...
	if (PMemory::Read_U32(infoPtr) != 0)
	{
		...
		m->nm.numWaitThreads = (int) m->waitingThreads.size();
		PMemory::WriteStruct(infoPtr, &m->nm);
	}
	return 0;
}
