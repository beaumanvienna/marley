...
static void __GameModeNotify(u64 userdata, int cyclesLate) {
	...
	if (IsGameModeActive()) {
			...
			if (ret >= 0 && bufsz > 0) {
				for (auto& gma : replicaGameModeAreas) {
					if (IsMatch(gma.mac, sendermac)) {
						...
						gma.dataUpdated = 1;
						gma.updateTimestamp = PCoreTiming::GetGlobalTimeUsScaled();
						break;
					}
				}
			}
		...
		// ReSchedule
		PCoreTiming::ScheduleEvent(usToCycles(GAMEMODE_UPDATE_INTERVAL) - cyclesLate, gameModeNotifyEvent, userdata);
		return;
	}
	...
	if (g_PConfig.bEnableWlan) {
		...
		if ((req.opcode == OPCODE_LOGIN && !networkInited) || (ret == SOCKET_ERROR && (sockerr == EAGAIN || sockerr == EWOULDBLOCK))) {
			...
			if (now - adhocctlStartTime <= static_cast<u64>(adhocDefaultTimeout) + 500) {
				...
				PCoreTiming::ScheduleEvent(usToCycles(500) - cyclesLate, adhocctlNotifyEvent, userdata);
				return;
			}
			else
...
int WaitBlockingAdhocctlSocket(AdhocctlRequest request, int usec, const char* reason) {
	...
	if (adhocctlNotifyEvent < 0)
		adhocctlNotifyEvent = PCoreTiming::RegisterEvent("__AdhocctlNotify", __AdhocctlNotify);

	u64 param = ((u64)__KernelGetCurThread()) << 32 | uid;
	adhocctlStartTime = (u64)(time_now_d() * 1000000.0);
	adhocctlRequests[uid] = request;
	PCoreTiming::ScheduleEvent(usToCycles(usec), adhocctlNotifyEvent, param);
...
int ScheduleAdhocctlState(int event, int newState, int usec, const char* reason) {
	...
	if (adhocctlStateEvent < 0)
		adhocctlStateEvent = PCoreTiming::RegisterEvent("__AdhocctlState", __AdhocctlState);

	u64 param = ((u64)__KernelGetCurThread()) << 32 | uid;
	PCoreTiming::ScheduleEvent(usToCycles(usec), adhocctlStateEvent, param);
...
int StartGameModeScheduler(int bufSize) {
	...
	if (gameModeNotifyEvent < 0)
		gameModeNotifyEvent = PCoreTiming::RegisterEvent("__GameModeNotify", __GameModeNotify);

	INFO_LOG(SCENET, "GameMode Scheduler (%d, %d) has started", gameModeSocket, bufSize);
	u64 param = ((u64)__KernelGetCurThread()) << 32 | bufSize;
	PCoreTiming::ScheduleEvent(usToCycles(GAMEMODE_INIT_DELAY), gameModeNotifyEvent, param);
		...
		if (ret > 0) {
			...
			if (resolveIP(sin.sin_addr.s_addr, &mac)) {
				...
				*req.length = ret;
				...
				if (peer != NULL) peer->last_recv = PCoreTiming::GetGlobalTimeUsScaled();
				peerlock.unlock();
			}
			// Unknown Peer
			else {
...
static void __AdhocSocketNotify(u64 userdata, int cyclesLate) {
	...
	case PDP_SEND:
		...
		if (DoBlockingPdpSend(uid, req, result, sendTargetPeers[userdata])) {
			...
			PCoreTiming::ScheduleEvent(usToCycles(delayUS) - cyclesLate, adhocSocketNotifyEvent, userdata);
			return;
		}
...
int WaitBlockingAdhocSocket(u64 threadSocketId, int type, int pspSocketId, void* buffer, s32_le* len, u32 timeoutUS, SceNetEtherAddr* remoteMAC, u16_le* remotePort, const char* reason) {
	...
	if (adhocSocketNotifyEvent < 0)
		adhocSocketNotifyEvent = PCoreTiming::RegisterEvent("__AdhocSocketNotify", __AdhocSocketNotify);
	...
	PCoreTiming::ScheduleEvent(usToCycles(100), adhocSocketNotifyEvent, threadSocketId);
...
void netAdhocValidateLoopMemory() {
	...
	if (!dummyThreadHackAddr || (dummyThreadHackAddr && strcmp("dummythreadhack", kernelMemory.GetBlockTag(dummyThreadHackAddr)) != 0)) {
		...
		if (dummyThreadHackAddr) PMemory::Memcpy(dummyThreadHackAddr, dummyThreadCode, sizeof(dummyThreadCode));
	}
	if (!matchingThreadHackAddr || (matchingThreadHackAddr && strcmp("matchingThreadHack", kernelMemory.GetBlockTag(matchingThreadHackAddr)) != 0)) {
		...
		if (matchingThreadHackAddr) PMemory::Memcpy(matchingThreadHackAddr, matchingThreadCode, sizeof(matchingThreadCode));
	}
}
...
void __NetAdhocDoState(PointerWrap &p) {
	...
	if (s >= 5) {
		...
		if (adhocctlNotifyEvent != -1) {
			PCoreTiming::RestoreRegisterEvent(adhocctlNotifyEvent, "__AdhocctlNotify", __AdhocctlNotify);
		}
		Do(p, adhocSocketNotifyEvent);
		if (adhocSocketNotifyEvent != -1) {
			PCoreTiming::RestoreRegisterEvent(adhocSocketNotifyEvent, "__AdhocSocketNotify", __AdhocSocketNotify);
		}
	}
	else {
	...
	if (s >= 6) {
		...
		if (gameModeNotifyEvent != -1) {
			PCoreTiming::RestoreRegisterEvent(gameModeNotifyEvent, "__GameModeNotify", __GameModeNotify);
		}
	}
	else {
	...
	if (s >= 7) {
		...
		if (adhocctlStateEvent != -1) {
			PCoreTiming::RestoreRegisterEvent(adhocctlStateEvent, "__AdhocctlState", __AdhocctlState);
		}
	}
	else {
...
u32_le __CreateHLELoop(u32_le *loopAddr, const char *sceFuncName, const char *hleFuncName, const char *tagName) {
	...
	PMemory::Memcpy(dummyThreadHackAddr, loopAddr, sizeof(u32_le) * 3); // This area will be cleared again after loading an old savestate :(
...
static u32 sceNetAdhocctlInit(int stackSize, int prio, u32 productAddr) {
	...
	if (PMemory::IsValidAddress(productAddr)) {
		PMemory::ReadStruct(productAddr, &product_code);
	}
	...
	if (g_PConfig.bEnableWlan && !networkInited) {
...
static int sceNetAdhocctlGetParameter(u32 paramAddr) {
	...
	if (netAdhocctlInited) {
		// Valid Arguments
		if (PMemory::IsValidAddress(paramAddr)) {
			// Copy Parameter
			PMemory::WriteStruct(paramAddr,&parameter);
			// Return Success
			return 0;
		}
			...
			if (sds[i].id > 0 && sds[i].id <= MAX_SOCKET && adhocSockets[sds[i].id - 1] != NULL) {
				...
				if (sock->type == SOCK_PTP) {
					...
					// Fate Unlimited Codes and Carnage Heart EXA relies on AdhocPollSocket in order to retry a failed PtpConnect, but the interval must not be too long (about 1 frame before state became Established by GetPtpStat) for Bleach Heat the Soul 7 to work properly.
						...
						(sock->data.ptp.state == ADHOC_PTP_STATE_SYN_SENT && (static_cast<s64>(PCoreTiming::GetGlobalTimeUsScaled() - sock->lastAttempt) > 1000/*std::max(1000, sock->retry_interval - 60000)*/)))) {
...
int sceNetAdhocPollSocket(u32 socketStructAddr, int count, int timeout, int nonblock) { // timeout in microseconds
	...
	if (netAdhocInited)
	{
		...
		if (PMemory::IsValidAddress(socketStructAddr)) sds = (SceNetAdhocPollSd *)PMemory::GetPointer(socketStructAddr);
...
static int sceNetAdhocctlGetAdhocId(u32 productStructAddr) {
	...
	{
		// Valid Arguments
		if (PMemory::IsValidAddress(productStructAddr))
		{
			// Copy Product ID
			PMemory::WriteStruct(productStructAddr, &product_code);
...
int sceNetAdhocctlGetScanInfo(u32 sizeAddr, u32 bufAddr) {
	s32_le *buflen = NULL;
	if (PMemory::IsValidAddress(sizeAddr)) buflen = (s32_le *)PMemory::GetPointer(sizeAddr);
	...
	INFO_LOG(SCENET, "sceNetAdhocctlGetScanInfo([%08x]=%i, %08x)", sizeAddr, PMemory::Read_U32(sizeAddr), bufAddr);
	if (!g_PConfig.bEnableWlan) {
		return 0;
	}
...
static u32 sceNetAdhocctlAddHandler(u32 handlerPtr, u32 handlerArg) {
	...
	if (!foundHandler && PMemory::IsValidAddress(handlerPtr)) {
...
static int sceNetAdhocctlGetNameByAddr(const char *mac, u32 nameAddr) {
	...
	{
		// Valid Arguments
		if (mac != NULL && PMemory::IsValidAddress(nameAddr))
		{
			SceNetAdhocctlNickname * nickname = (SceNetAdhocctlNickname *)PMemory::GetPointer(nameAddr);
...
int sceNetAdhocctlGetPeerInfo(const char *mac, int size, u32 peerInfoAddr) {
	...
	if (PMemory::IsValidAddress(peerInfoAddr)) {
		buf = (SceNetAdhocctlPeerInfoEmu *)PMemory::GetPointer(peerInfoAddr);
	}
	// Library initialized
	if (netAdhocctlInited) {
		...
		if (isLocalMAC(maddr)) {
			...
			truncate_cpy((char*)&nickname.data, ADHOCCTL_NICKNAME_LEN, g_PConfig.sNickName.c_str());
			buf->next = 0;
			...
			buf->flags = 0x0400;
			buf->padding = 0;
			buf->last_recv = std::max(0LL, static_cast<s64>(PCoreTiming::GetGlobalTimeUsScaled() - defaultLastRecvDelta));
		...
		else 
		{
			...
			if (peer != NULL && peer->last_recv != 0) {
				// Fake Receive Time
				peer->last_recv = std::max(peer->last_recv, PCoreTiming::GetGlobalTimeUsScaled() - defaultLastRecvDelta);
				...
				buf->padding = 0;
				buf->last_recv = peer->last_recv; //PCoreTiming::GetGlobalTimeUsScaled(); //time_now_d()*1000000.0; //(uint64_t)time(NULL); //This timestamp is important issue on Dissidia 012
			...
			if ((adhocctlState == ADHOCCTL_STATE_DISCONNECTED) || (adhocctlState == ADHOCCTL_STATE_SCANNING)) {
				...
				if (iResult == SOCKET_ERROR && error != EAGAIN && error != EWOULDBLOCK) {
					...
					if (adhocctlCurrentMode == ADHOCCTL_MODE_GAMEMODE) {
						...
						notifyAdhocctlHandlers(ADHOCCTL_EVENT_CONNECT, 0); //PCoreTiming::ScheduleEvent_Threadsafe_Immediate(eventAdhocctlHandlerUpdate, join32(ADHOCCTL_EVENT_CONNECT, 0)); 
					}
				}
...
int sceNetAdhocctlJoin(u32 scanInfoAddr) {
	...
	{
		// Valid Argument
		if (PMemory::IsValidAddress(scanInfoAddr))
		{
			SceNetAdhocctlScanInfoEmu* sinfo = (SceNetAdhocctlScanInfoEmu*)PMemory::GetPointer(scanInfoAddr);
			char grpName[ADHOCCTL_GROUPNAME_LEN + 1] = { 0 };
...
int NetAdhocctl_CreateEnterGameMode(const char* group_name, int game_type, int num_members, u32 membersAddr, u32 timeout, int flag) {
	...
	if (!PMemory::IsValidAddress(membersAddr))
		return ERROR_NET_ADHOCCTL_INVALID_ARG;
...
static int sceNetAdhocGetPdpStat(u32 structSize, u32 structAddr) {
	...
	{
		s32_le *buflen = NULL;
		if (PMemory::IsValidAddress(structSize)) buflen = (s32_le *)PMemory::GetPointer(structSize);
		SceNetAdhocPdpStat *buf = NULL;
		if (PMemory::IsValidAddress(structAddr)) buf = (SceNetAdhocPdpStat *)PMemory::GetPointer(structAddr);
...
static int sceNetAdhocGetPtpStat(u32 structSize, u32 structAddr) {
	...
	if (PMemory::IsValidAddress(structAddr)) buf = (SceNetAdhocPtpStat *)PMemory::GetPointer(structAddr);
		...
		// Status Returner Mode
			...
			for (int j = 0; j < MAX_SOCKET && i < count; j++) {
				...
				if ( sock != NULL && sock->type == SOCK_PTP) {
					...
					if ((sock->data.ptp.state == ADHOC_PTP_STATE_SYN_SENT || sock->data.ptp.state == ADHOC_PTP_STATE_SYN_RCVD) && (static_cast<s64>(PCoreTiming::GetGlobalTimeUsScaled() - sock->lastAttempt) > 35000/*sock->retry_interval*/)) {
...
static int sceNetAdhocPtpAccept(int id, u32 peerMacAddrPtr, u32 peerPortPtr, int timeout, int flag) {
	...
	if (PMemory::IsValidAddress(peerMacAddrPtr)) {
		addr = PSPPointer<SceNetEtherAddr>::Create(peerMacAddrPtr);
	}
	uint16_t * port = NULL; //
	if (PMemory::IsValidAddress(peerPortPtr)) {
		port = (uint16_t *)PMemory::GetPointer(peerPortPtr);
	}
	if (flag == 0) { // Prevent spamming Debug Log with retries of non-bocking socket
			...
			if (ptpsocket.state == ADHOC_PTP_STATE_CLOSED || ptpsocket.state == ADHOC_PTP_STATE_SYN_SENT) {
				...
				if (resolveMAC(&ptpsocket.paddr, (uint32_t *)&sin.sin_addr.s_addr)) {
					...
					if (connectresult != SOCKET_ERROR || errorcode == EISCONN) {
						socket->attemptCount++;
						socket->lastAttempt = PCoreTiming::GetGlobalTimeUsScaled();
						// Set Connected State
						ptpsocket.state = ADHOC_PTP_STATE_ESTABLISHED;
					...
					// Error handling
						...
						if (connectInProgress(errorcode)) {
							...
							if (!flag || (g_PConfig.bForcedFirstConnect && socket->attemptCount == 1)) {
								// Simulate blocking behaviour with non-blocking socket
								u64 threadSocketId = ((u64)__KernelGetCurThread()) << 32 | ptpsocket.id;
...
static int sceNetAdhocPtpSend(int id, u32 dataAddr, u32 dataSizeAddr, int timeout, int flag) {
	...
	const char * data = PMemory::GetCharPointer(dataAddr);
	// Library is initialized
	if (netAdhocInited) {
		// Valid Socket
		if (id > 0 && id <= MAX_SOCKET && adhocSockets[id - 1] != NULL) {
...
static int sceNetAdhocGameModeCreateMaster(u32 dataAddr, int size) {
	...
	if (size < 0 || !PMemory::IsValidAddress(dataAddr))
		return hleLogError(SCENET, ERROR_NET_ADHOCCTL_INVALID_ARG, "invalid arg");
	...
	if (data) {
		PMemory::Memcpy(data, dataAddr, size);
		masterGameModeArea = { 0, size, dataAddr, PCoreTiming::GetGlobalTimeUsScaled(), 1, localMac, data };
...
static int sceNetAdhocGameModeCreateReplica(const char *mac, u32 dataAddr, int size) {
	...
	if (mac == nullptr || size < 0 || !PMemory::IsValidAddress(dataAddr))
		return hleLogError(SCENET, ERROR_NET_ADHOCCTL_INVALID_ARG, "invalid arg");
	...
	if (data) {
		...
		GameModeArea gma = { maxid + 1, size, dataAddr, PCoreTiming::GetGlobalTimeUsScaled(), 0, *(SceNetEtherAddr*)mac, data };
...
static int sceNetAdhocGameModeUpdateMaster() {
	...
	if (masterGameModeArea.data) {
		PMemory::Memcpy(masterGameModeArea.data, masterGameModeArea.addr, masterGameModeArea.size);
		masterGameModeArea.dataUpdated = 1;
		masterGameModeArea.updateTimestamp = PCoreTiming::GetGlobalTimeUsScaled();
	}
	...
	for (auto gma : replicaGameModeAreas) {
		if (gma.id == id) {
			if (PMemory::IsValidAddress(infoAddr)) {
				GameModeUpdateInfo* gmuinfo = (GameModeUpdateInfo*)PMemory::GetPointer(infoAddr);
				gmuinfo->length = sizeof(GameModeUpdateInfo);
				if (gma.data && gma.dataUpdated) {
					memcpy(PMemory::GetPointer(gma.addr), gma.data, gma.size);
					gma.dataUpdated = 0;
					gmuinfo->updated = 1;
					gmuinfo->timeStamp = std::max(gma.updateTimestamp, PCoreTiming::GetGlobalTimeUsScaled() - 1000);
				}
				else {
					gmuinfo->updated = 0;
				}
...
int sceNetAdhocGetSocketAlert(int id, u32 flagPtr) {
	...
	if (!PMemory::IsValidAddress(flagPtr))
		return hleLogDebug(SCENET, ERROR_NET_ADHOC_INVALID_ARG, "invalid arg");
			...
			if (context != NULL)
			{
				...
				if (context->running)
				{
					...
					if (peer != NULL)
					{
						...
						{
							void * opt = NULL;
							if (PMemory::IsValidAddress(optDataPtr)) opt = PMemory::GetPointer(optDataPtr);
							// Host Mode
...
int sceNetAdhocMatchingGetHelloOpt(int matchingId, u32 optLenAddr, u32 optDataAddr) {
	...
	if (item != NULL) {
		...
		*optlen = item->hellolen;
		if ((*optlen > 0) && PMemory::IsValidAddress(optDataAddr)) {
			uint8_t * optdata = PMemory::GetPointer(optDataAddr);
...
int sceNetAdhocMatchingSetHelloOpt(int matchingId, int optLenAddr, u32 optDataAddr) {
	...
	if (optLenAddr > 0)
	{
		...
		// Clone Hello Data
		PMemory::Memcpy(hello, optDataAddr, optLenAddr);
...
static int sceNetAdhocMatchingGetMembers(int matchingId, u32 sizeAddr, u32 buf) {
	...
	int* buflen = (int*)PMemory::GetPointer(sizeAddr);
	SceNetAdhocMatchingMemberInfoEmu* buf2 = NULL;
	if (PMemory::IsValidAddress(buf)) {
		buf2 = (SceNetAdhocMatchingMemberInfoEmu*)PMemory::GetPointer(buf);
	}
				...
				if (context->mode == PSP_ADHOC_MATCHING_MODE_P2P)
				{
					...
					if (p2p != NULL)
					{
						...
						if (p2p->lastping != 0 && friendpeer != NULL && friendpeer->last_recv != 0)
							p2p->lastping = std::max(p2p->lastping, PCoreTiming::GetGlobalTimeUsScaled() - defaultLastRecvDelta);
						else
							p2p->lastping = 0;
				...
				else
				{
					...
					if (parentpeer != NULL) {
						...
						if (parentpeer->lastping != 0 && friendpeer != NULL && friendpeer->last_recv != 0)
							parentpeer->lastping = std::max(parentpeer->lastping, PCoreTiming::GetGlobalTimeUsScaled() - defaultLastRecvDelta);
						else
					...
					for (; peer != NULL && filledpeers < requestedpeers; peer = peer->next)
					{
						...
						if (!excludeTimedout || peer->lastping != 0) {
							...
							if (peer->lastping != 0 && friendpeer != NULL && friendpeer->last_recv != 0)
								peer->lastping = std::max(peer->lastping, PCoreTiming::GetGlobalTimeUsScaled() - defaultLastRecvDelta);
							else
								peer->lastping = 0;
		...
		if (mac != NULL)
		{
			...
			if (context != NULL)
			{
				...
				if (context->running)
				{
					...
					void* data = NULL;
					if (PMemory::IsValidAddress(dataAddr)) data = PMemory::GetPointer(dataAddr);
...
int sceNetAdhocMatchingGetPoolStat(u32 poolstatPtr) {
	...
	if (netAdhocMatchingInited)
	{
		...
		if (PMemory::IsValidAddress(poolstatPtr)) poolstat = (SceNetMallocStat *)PMemory::GetPointer(poolstatPtr);
...
void __NetMatchingCallbacks() //(int matchingId)
{
	...
	if (params != matchingEvents.end())
	{
		...
		DEBUG_LOG(SCENET, "AdhocMatchingCallback: [ID=%i][EVENT=%i][%s]", args[0], args[1], mac2str((SceNetEtherAddr*)PMemory::GetPointer(args[2])).c_str());
...
static int sceNetAdhocctlGetGameModeInfo(u32 infoAddr) {
	...
	SceNetAdhocctlGameModeInfo* gmInfo = (SceNetAdhocctlGameModeInfo*)PMemory::GetPointer(infoAddr);
			...
			else
			{
				...
				if (requestcount > 0)
				{
					...
					if (strncmp((char *)&parameter.nickname.data, nickName, ADHOCCTL_NICKNAME_LEN) == 0)
					{
						...
						buf[discovered].flags = 0x0400;
						u64 lastrecv = std::max(0LL, static_cast<s64>(PCoreTiming::GetGlobalTimeUsScaled() - defaultLastRecvDelta));
...
void actOnPingPacket(SceNetAdhocMatchingContext * context, SceNetEtherAddr * sendermac)
{
	...
	if (peer != NULL)
	{
		...
		peer->lastping = PCoreTiming::GetGlobalTimeUsScaled(); //time_now_d()*1000000.0;
	}
}
	...
	if (peer != NULL && context->mode == PSP_ADHOC_MATCHING_MODE_CHILD && peer == findParent(context))
	{
		...
		if (length >= (1 + sizeof(SceNetEtherAddr)))
		{
			...
			if (sibling != NULL)
			{
				...
				// Initialize Ping Timer
				sibling->lastping = PCoreTiming::GetGlobalTimeUsScaled(); //time_now_d()*1000000.0;
...
int matchingInputThread(int matchingId) // TODO: The MatchingInput thread is using sceNetAdhocPdpRecv & sceNetAdhocPdpSend functions so it might be better to run this on PSP thread instead of real thread
{
	...
	if (context != NULL) {
		while (contexts != NULL && context->inputRunning)
		{
			...
			if (context != NULL) {
				now = PCoreTiming::GetGlobalTimeUsScaled(); //time_now_d()*1000000.0;
				...
				if (recvresult == 0 && rxbuflen > 0 && context->port == senderport)
				{
					...
					if (peer != NULL) {
						now = PCoreTiming::GetGlobalTimeUsScaled();
