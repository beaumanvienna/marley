...
static void __JpegCsc(u32 imageAddr, u32 yCbCrAddr, int widthHeight, int bufferWidth) {
	...
	u32 *imageBuffer = (u32*)PMemory::GetPointer(imageAddr);
	int sizeY = width * height;
	int sizeCb = sizeY >> 2;
	u8 *Y = (u8*)PMemory::GetPointer(yCbCrAddr);
	u8 *Cb = Y + sizeY;
	u8 *Cr = Cb + sizeCb;
...
static int __DecodeJpeg(u32 jpegAddr, int jpegSize, u32 imageAddr) {
	u8 *buf = PMemory::GetPointer(jpegAddr);
	...
	if (actual_components == 3) {
			...
			u32 *abgr = (u32*)PMemory::GetPointer(imageAddr);
			int pspWidth = 0;
...
static int sceJpegDecodeMJpeg(u32 jpegAddr, int jpegSize, u32 imageAddr, int dhtMode) {
	if (!PMemory::IsValidAddress(jpegAddr)) {
...
static int __JpegGetOutputInfo(u32 jpegAddr, int jpegSize, u32 colourInfoAddr) {
	...
	if (PMemory::IsValidAddress(colourInfoAddr)) {
		PMemory::Write_U32(0x00020202, colourInfoAddr);
	}

#ifdef JPEG_DEBUG
		char jpeg_fname[256];
		u8 *jpegDumpBuf = PMemory::GetPointer(jpegAddr);
