...
static int sceNpGetContentRatingFlag(u32 parentalControlAddr, u32 userAgeAddr)
{
	...
	if (!PMemory::IsValidAddress(parentalControlAddr) || !PMemory::IsValidAddress(userAgeAddr))
		return hleLogError(HLE, SCE_NP_ERROR_INVALID_ARGUMENT, "invalid arg");

	PMemory::Write_U32(parentalControl, parentalControlAddr);
	PMemory::Write_U32(userAge, userAgeAddr);
...
static int sceNpGetChatRestrictionFlag(u32 flagAddr)
{
	...
	if (!PMemory::IsValidAddress(flagAddr))
		return hleLogError(HLE, SCE_NP_ERROR_INVALID_ARGUMENT, "invalid arg");

	PMemory::Write_U32(chatRestriction, flagAddr);
...
static int sceNpAuthGetMemoryStat(u32 memStatAddr)
{
	...
	if (!PMemory::IsValidAddress(memStatAddr))
		return hleLogError(HLE, SCE_NP_AUTH_ERROR_INVALID_ARGUMENT, "invalid arg");

	PMemory::WriteStruct(memStatAddr, &npAuthMemStat);
...
static int sceNpAuthCreateStartRequest(u32 paramAddr)
{
	...
	PMemory::Memcpy(&params, paramAddr, size);
	serviceId = PMemory::GetCharPointer(params.serviceIdAddr);

	INFO_LOG(HLE, "%s - Max Version: %u.%u", __FUNCTION__, params.version.major, params.version.minor);
	INFO_LOG(HLE, "%s - Service ID: %s", __FUNCTION__, PMemory::GetCharPointer(params.serviceIdAddr));
	INFO_LOG(HLE, "%s - Entitlement ID: %s", __FUNCTION__, PMemory::GetCharPointer(params.entitlementIdAddr));
	INFO_LOG(HLE, "%s - Cookie (size = %d): %s", __FUNCTION__, params.cookieSize, PMemory::GetCharPointer(params.cookieAddr));
	...
	if (params.size >= 32 && params.ticketCbAddr != 0) {
		...
		if (!foundHandler && PMemory::IsValidAddress(handler.entryPoint)) {
...
static int sceNpAuthGetTicket(u32 requestId, u32 bufferAddr, u32 length)
{
	...
	int result = length;
	PMemory::Memset(bufferAddr, 0, length);
	SceNpTicket ticket = {};
	...
	u8* buf = PMemory::GetPointer(bufferAddr + sizeof(ticket));
	int ofs = 0;
	...
	PMemory::WriteStruct(bufferAddr, &ticket);
	...
	PMemory::WriteStruct(bufferAddr + sizeof(ticket) + ofs, &footer);
	ofs += sizeof(footer);
	...
	u8 unknownBytes[36] = {}; 
	PMemory::WriteStruct(bufferAddr + sizeof(ticket) + ofs, unknownBytes);
...
static int sceNpAuthGetEntitlementById(u32 ticketBufferAddr, u32 ticketLength, u32 entitlementIdAddr, u32 arg4)
{
	...
	INFO_LOG(HLE, "%s - Entitlement ID: %s", __FUNCTION__, PMemory::GetCharPointer(entitlementIdAddr));
...
static int sceNpLookupCreateTransactionCtx(u32 lookupTitleCtxIdAddr)
{
	...
	INFO_LOG(SCENET, "%s - Title ID: %s", __FUNCTION__, PMemory::GetCharPointer(lookupTitleCtxIdAddr));
