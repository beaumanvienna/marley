	...
	case PSP_FONT_PIXELFORMAT_4_REV:
		{
			...
			int oldColor = PMemory::Read_U8(framebufferAddr);
			int newColor;
			if ((x & 1) != pixelformat) {
			...
			PMemory::Write_U8(newColor, framebufferAddr);
			break;
		}
	case PSP_FONT_PIXELFORMAT_8:
		{
			PMemory::Write_U8(pixelColor, framebufferAddr);
			break;
		}
	case PSP_FONT_PIXELFORMAT_24:
		{
			// Each channel has the same value.
			PMemory::Write_U8(pixelColor, framebufferAddr + 0);
			PMemory::Write_U8(pixelColor, framebufferAddr + 1);
			PMemory::Write_U8(pixelColor, framebufferAddr + 2);
			break;
		}
	case PSP_FONT_PIXELFORMAT_32:
		{
			...
			pix32 |= pix32 << 8;
			pix32 |= pix32 << 16;
			PMemory::Write_U32(pix32, framebufferAddr);
			break;
		}
	}
}
