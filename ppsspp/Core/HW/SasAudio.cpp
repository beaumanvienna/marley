...
void VagDecoder::GetSamples(s16 *outSamples, int numSamples) {
	...
	if (!Memory_P::IsValidAddress(read_)) {
		WARN_LOG(SASMIX, "Bad VAG samples address?");
		return;
	}
	u8 *readp = Memory_P::GetPointerUnchecked(read_);
	u8 *origp = readp;
	...
	for (int i = 0; i < numSamples; i++) {
		if (curSample == 28) {
			if (loopAtNextBlock_) {
				...
				readp = Memory_P::GetPointerUnchecked(read_);
				origp = readp;
	...
	case VOICETYPE_PCM:
		{
			...
			while (needed > 0) {
				...
				Memory_P::Memcpy(out, pcmAddr + pcmIndex * sizeof(s16), size * sizeof(s16));
				pcmIndex += size;
				needed -= size;
				out += size;
				if (pcmIndex >= pcmSize) {
...
void SasInstance::Mix(u32 outAddr, u32 inAddr, int leftVol, int rightVol) {
	...
	s16 *outp = (s16 *)Memory_P::GetPointer(outAddr);
	const s16 *inp = inAddr ? (s16*)Memory_P::GetPointer(inAddr) : 0;
...
#ifdef AUDIO_TO_FILE
	fwrite(Memory_P::GetPointer(outAddr), 1, grainSize * 2 * 2, audioDump);
#endif
}
