...
namespace MIPSStackWalk {
	...
	bool ScanForAllocaSignature(u32 pc) {
		...
		u32 stop = pc - 32 * 4;
		for (; PMemory::IsValidAddress(pc) && pc >= stop; pc -= 4) {
			MIPSOpcode op = PMemory::Read_Instruction(pc, true);
	...
	bool ScanForEntry(StackFrame &frame, u32 entry, u32 &ra) {
		...
		for (u32 pc = start; PMemory::IsValidAddress(pc) && pc >= stop; pc -= 4) {
			MIPSOpcode op = PMemory::Read_Instruction(pc, true);
			...
			if (IsAddImmInstr(op) && _RT == MIPS_REG_SP && _RS == MIPS_REG_SP) {
				...
				if (ra_offset != -1 && PMemory::IsValidAddress(frame.sp + ra_offset)) {
					ra = PMemory::Read_U32(frame.sp + ra_offset);
				}
				return true;
			}
		}
