...
int PSPNetconfDialog::Init(u32 paramAddr) {
	...
	PMemory::Memcpy(&request, paramAddr, size);
...
int PSPNetconfDialog::Update(int animSpeed) {
	...
	if (request.netAction == NETCONF_CONNECT_APNET || request.netAction == NETCONF_STATUS_APNET || request.netAction == NETCONF_CONNECT_APNET_LAST) {
		...
		if (!hideNotice) {
			...
			const ImageID confirmBtnImage = g_PConfig.iButtonPreference == PSP_SYSTEMPARAM_BUTTON_CROSS ? ImageID("I_CROSS") : ImageID("I_CIRCLE");
			const int confirmBtn = g_PConfig.iButtonPreference == PSP_SYSTEMPARAM_BUTTON_CROSS ? CTRL_CROSS : CTRL_CIRCLE;
			const ImageID cancelBtnImage = g_PConfig.iButtonPreference == PSP_SYSTEMPARAM_BUTTON_CROSS ? ImageID("I_CIRCLE") : ImageID("I_CROSS");
			const int cancelBtn = g_PConfig.iButtonPreference == PSP_SYSTEMPARAM_BUTTON_CROSS ? CTRL_CIRCLE : CTRL_CROSS;
		...
		if (timedout) {
			...
			DisplayMessage(di->T("InternalError", "An internal error has occurred.") + PStringFromFormat("\n(%08X)", connResult));
			DisplayButtons(DS_BUTTON_CANCEL, di->T("Back"));
		}
		else {
			std::string channel = std::to_string(g_PConfig.iWlanAdhocChannel);
			if (g_PConfig.iWlanAdhocChannel == PSP_SYSTEMPARAM_ADHOC_CHANNEL_AUTOMATIC)
				channel = "Automatic";
					...
					case NETCONF_JOIN_ADHOC:
						...
						if (scanStep == 0) {
							if (sceNetAdhocctlScan() >= 0) {
								...
								if (PMemory::IsValidAddress(scanInfosAddr))
									userMemory.Free(scanInfosAddr);
								scanInfosAddr = userMemory.Alloc(structsz, false, "NetconfScanInfo");
								PMemory::Write_U32(sizeof(SceNetAdhocctlScanInfoEmu), scanInfosAddr);
								scanStep = 1;
							}
						}
						else if (scanStep == 1) {
							s32 sz = PMemory::Read_U32(scanInfosAddr);
							// Get required buffer size
							if (sceNetAdhocctlGetScanInfo(scanInfosAddr, 0) >= 0) {
								s32 reqsz = PMemory::Read_U32(scanInfosAddr);
								if (reqsz > sz) {
									sz = reqsz;
									if (PMemory::IsValidAddress(scanInfosAddr))
										userMemory.Free(scanInfosAddr);
									...
									scanInfosAddr = userMemory.Alloc(structsz, false, "NetconfScanInfo");
									PMemory::Write_U32(sz, scanInfosAddr);
								}
								if (reqsz > 0) {
									if (sceNetAdhocctlGetScanInfo(scanInfosAddr, scanInfosAddr + sizeof(s32)) >= 0) {
										ScanInfos* scanInfos = (ScanInfos*)PMemory::GetPointer(scanInfosAddr);
										...
										while (n > 0) {
											SceNetAdhocctlScanInfoEmu* si = (SceNetAdhocctlScanInfoEmu*)PMemory::GetPointer(scanInfosAddr + sizeof(s32) + sizeof(SceNetAdhocctlScanInfoEmu) * (n - 1LL));
	...
	if (GetStatus() == SCE_UTILITY_STATUS_FINISHED || pendingStatus == SCE_UTILITY_STATUS_FINISHED)
		PMemory::Memcpy(requestAddr, &request, request.common.size);
