...
static void InitSDLAudioDevice() {
	...
	if (!g_PConfig.sAudioDevice.empty()) {
		...
		if (audioDev <= 0) {
			WARN_LOG(AUDIO, "Failed to open preferred audio device %s", g_PConfig.sAudioDevice.c_str());
		}
	}
	if (audioDev <= 0) {
...
void MainUI::initializeGL() {
	...
	if (g_PConfig.iGPUBackend == (int)GPUBackend::OPENGL) {
		...
		INFO_LOG(SYSTEM, "Not using thread, backend=%d", (int)g_PConfig.iGPUBackend);
	}
