...
static const D3DVERTEXELEMENT9 g_FramebufferVertexElements[] = {
		...
		if (offscreen) {
			...
			if (SUCCEEDED(hr)) {
				...
				if (SUCCEEDED(hr)) {
					...
					ConvertFromBGRA8888(PMemory::GetPointer(fb_address + dstByteOffset), (u8 *)locked.pBits, vfb->fb_stride, locked.Pitch / 4, w, h, vfb->format);
	...
	void FramebufferManagerDX9::PackDepthbuffer(VirtualFramebuffer *vfb, int x, int y, int w, int h) {
		...
		if (tex) {
			...
			if (SUCCEEDED(hr)) {
				...
				u16 *depth = (u16 *)PMemory::GetPointer(z_address);
	...
	bool FramebufferManagerDX9::GetFramebuffer(u32 fb_address, int fb_stride, GEBufferFormat fb_format, GPUDebugBuffer &buffer, int maxRes) {
		...
		if (!vfb) {
			...
			buffer = GPUDebugBuffer(PMemory::GetPointer(fb_address), fb_stride, 512, fb_format);
			return true;
		}
	...
	bool FramebufferManagerDX9::GetStencilbuffer(u32 fb_address, int fb_stride, GPUDebugBuffer &buffer) {
		...
		if (!vfb) {
			// If there's no vfb and we're drawing there, must be memory?
			buffer = GPUDebugBuffer(PMemory::GetPointer(vfb->z_address), vfb->z_stride, 512, GPU_DBG_FORMAT_16BIT);
			return true;
		}
