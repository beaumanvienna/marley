...
void TextureCacheVulkan::DeviceRestore(VulkanContext *vulkan, Draw::DrawContext *draw) {
	...
	VkResult res = PvkCreateSampler(vulkan_->GetDevice(), &samp, nullptr, &samplerNearest_);
	_assert_(res == VK_SUCCESS);
...
void TextureCacheVulkan::CompileScalingShader() {
	if (!g_PConfig.bTexHardwareScaling || g_PConfig.sTextureShaderName != textureShader_) {
		if (uploadCS_ != VK_NULL_HANDLE)
			vulkan_->Delete().QueueDeleteShaderModule(uploadCS_);
		if (copyCS_ != VK_NULL_HANDLE)
			vulkan_->Delete().QueueDeleteShaderModule(copyCS_);
	...
	const TextureShaderInfo *shaderInfo = GetTextureShaderInfo(g_PConfig.sTextureShaderName);
	if (!shaderInfo || shaderInfo->computeShaderFile.empty())
		return;

	std::string shaderSource = ReadShaderSrc(shaderInfo->computeShaderFile);
	std::string fullUploadShader = PStringFromFormat(uploadShader, shaderSource.c_str());
	std::string fullCopyShader = PStringFromFormat(copyShader, shaderSource.c_str());
	...
	textureShader_ = g_PConfig.sTextureShaderName;
}
...
void TextureCacheVulkan::ApplyTextureFramebuffer(VirtualFramebuffer *framebuffer, GETextureFormat texFormat, FramebufferNotificationChannel channel) {
	...
	if (need_depalettize && !g_PConfig.bDisableSlowFramebufEffects) {
		if (useShaderDepal) {
...
void TextureCacheVulkan::BuildTexture(TexCacheEntry *const entry) {
	...
	bool hardwareScaling = g_PConfig.bTexHardwareScaling && (uploadCS_ != VK_NULL_HANDLE || copyCS_ != VK_NULL_HANDLE);
