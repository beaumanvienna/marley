...
bool GenerateVertexShaderVulkanGLSL(const VShaderID &id, char *buffer, std::string *errorString) {
	...
	if (!useHWTransform) {
		...
		if (doTexture) {
			...
			{
				std::string lightFactor0 = PStringFromFormat("(length(u_lightpos%d) == 0.0 ? worldnormal.z : dot(normalize(u_lightpos%d), worldnormal))", ls0, ls0);
				std::string lightFactor1 = PStringFromFormat("(length(u_lightpos%d) == 0.0 ? worldnormal.z : dot(normalize(u_lightpos%d), worldnormal))", ls1, ls1);
