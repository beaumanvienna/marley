...
SamplerCacheKey TextureCacheCommon::GetSamplingParams(int maxLevel, u32 texAddr) {
	...
	// Filtering overrides
	switch (g_PConfig.iTexFiltering) {
	case TEX_FILTER_AUTO:
		...
		if (gstate.isModeThrough() && g_PConfig.iInternalResolution != 1) {
...
TexCacheEntry *TextureCacheCommon::SetTexture() {
	...
	if (!entry) {
		...
		if (PMemory::IsKernelAndNotVolatileAddress(texaddr)) {
			// It's the builtin font texture.
			entry->status = TexCacheEntry::STATUS_RELIABLE;
		} else if (g_PConfig.bTextureBackoffCache) {
...
std::vector<AttachCandidate> TextureCacheCommon::GetFramebufferCandidates(const TextureDefinition &entry, u32 texAddrOffset) {
	...
	FramebufferNotificationChannel channel = PMemory::IsDepthTexVRAMAddress(entry.addr) ? FramebufferNotificationChannel::NOTIFY_FB_DEPTH : FramebufferNotificationChannel::NOTIFY_FB_COLOR;
...
FramebufferMatchInfo TextureCacheCommon::MatchFramebuffer(
	...
	bool texInVRAM = PMemory::IsVRAMAddress(texaddr);
	bool fbInVRAM = PMemory::IsVRAMAddress(fb_address);
...
void TextureCacheCommon::ApplyTexture() {
	...
	if (!entry) {
		...
		InvalidateLastTexture();
		if (nextFramebufferTexture_) {
			bool depth = PMemory::IsDepthTexVRAMAddress(gstate.getTextureAddress(0));
