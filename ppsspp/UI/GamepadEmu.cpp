...
float GamepadView::GetButtonOpacity() {
	float fadeAfterSeconds = g_PConfig.iTouchButtonHideSeconds;
...
void MultiTouchButton::Draw(UIContext &dc) {
	...
	float scale = scale_;
	if (IsDown()) {
		if (g_PConfig.iTouchButtonStyle == 2) {
			opacity *= 1.35f;
		} else {
			scale *= 2.0f;
			opacity *= 1.15f;
	...
	if (IsDown() && g_PConfig.iTouchButtonStyle == 2) {
...
void FPSLimitButton::Touch(const TouchInput &input) {
	...
	if (!down && lastDown && IsDown()) {
		...
		int limit = limit_ == FPSLimit::CUSTOM1 ? g_PConfig.iFpsLimit1 : g_PConfig.iFpsLimit2;
...
void AnalogRotationButton::Update() {
	...
	if (autoRotating_) {
		float speed = clockWise_ ? -g_PConfig.fAnalogAutoRotSpeed : g_PConfig.fAnalogAutoRotSpeed;
		// Clamp to a square
...
void PSPDpad::ProcessTouch(float x, float y, bool down) {
	...
	bool fourWay = g_PConfig.bDisableDpadDiagonals || rad < 0.7f;
	if (down) {
		if (fourWay) {
...
void PSPStick::Draw(UIContext &dc) {
	...
	if (dragPointerId_ != -1 && g_PConfig.iTouchButtonStyle == 2) {
		opacity *= 1.35f;
	}
	...
	if (dragPointerId_ != -1 && g_PConfig.iTouchButtonStyle == 2 && stickDownImg_ != stickImageIndex_)
...
void PSPStick::Touch(const TouchInput &input) {
	...
	if (input.flags & TOUCH_DOWN) {
		if (dragPointerId_ == -1 && bounds_.Contains(input.x, input.y)) {
			if (g_PConfig.bAutoCenterTouchAnalog) {
				centerX_ = input.x;
				centerY_ = input.y;
			} else {
				centerX_ = bounds_.centerX();
...
void PSPCustomStick::ProcessTouch(float x, float y, bool down) {
	...
	if (down && centerX_ >= 0.0f) {
		...
		if (g_PConfig.iRightAnalogRight != 0) {
			if (dx > 0.5f)
				__CtrlButtonDown(button[g_PConfig.iRightAnalogRight-1]);
			else
				__CtrlButtonUp(button[g_PConfig.iRightAnalogRight-1]);
		}
		if (g_PConfig.iRightAnalogLeft != 0) {
			if (dx < -0.5f)
				__CtrlButtonDown(button[g_PConfig.iRightAnalogLeft-1]);
			else
				__CtrlButtonUp(button[g_PConfig.iRightAnalogLeft-1]);
		}
		if (g_PConfig.iRightAnalogUp != 0) {
			if (dy < -0.5f)
				__CtrlButtonDown(button[g_PConfig.iRightAnalogUp-1]);
			else
				__CtrlButtonUp(button[g_PConfig.iRightAnalogUp-1]);
		}
		if (g_PConfig.iRightAnalogDown != 0) {
			if (dy > 0.5f)
				__CtrlButtonDown(button[g_PConfig.iRightAnalogDown-1]);
			else
				__CtrlButtonUp(button[g_PConfig.iRightAnalogDown-1]);
		}
		if (g_PConfig.iRightAnalogPress != 0)
			__CtrlButtonDown(button[g_PConfig.iRightAnalogPress-1]);
		...
		if (g_PConfig.iRightAnalogRight != 0)
			__CtrlButtonUp(button[g_PConfig.iRightAnalogRight-1]);
		if (g_PConfig.iRightAnalogPress != 0)
			__CtrlButtonUp(button[g_PConfig.iRightAnalogPress-1]);
...
void InitPadLayout(float xres, float yres, float globalScale) {
	...
	if (g_PConfig.fActionButtonSpacing < 0) {
		g_PConfig.fActionButtonSpacing = 1.0f;
	}

	// Position of the circle button (the PSP circle button). It is the farthest to the left
	float Action_button_spacing = g_PConfig.fActionButtonSpacing * baseActionButtonSpacing;
	...
	initTouchPos(g_PConfig.touchActionButtonCenter, Action_button_center_X, Action_button_center_Y);
	...
	initTouchPos(g_PConfig.touchDpad, D_pad_X, D_pad_Y);

	//analog stick-------------------------------------------------------
	//keep the analog stick right below the D pad
	int analog_stick_X = D_pad_X;
	int analog_stick_Y = yres - 80 * scale;
	initTouchPos(g_PConfig.touchAnalogStick, analog_stick_X, analog_stick_Y);
	...
	initTouchPos(g_PConfig.touchRightAnalogStick, right_analog_stick_X, right_analog_stick_Y);
	...
	initTouchPos(g_PConfig.touchStartKey, start_key_X, start_key_Y);

	int select_key_X = halfW;
	int select_key_Y = yres - 60 * scale;
	initTouchPos(g_PConfig.touchSelectKey, select_key_X, select_key_Y);

	int unthrottle_key_X = halfW - bottom_key_spacing * scale;
	int unthrottle_key_Y = yres - 60 * scale;
	initTouchPos(g_PConfig.touchUnthrottleKey, unthrottle_key_X, unthrottle_key_Y);

	initTouchPos(g_PConfig.touchSpeed1Key, unthrottle_key_X, unthrottle_key_Y - 60 * scale);
	initTouchPos(g_PConfig.touchSpeed2Key, unthrottle_key_X + bottom_key_spacing * scale, unthrottle_key_Y - 60 * scale);
	initTouchPos(g_PConfig.touchRapidFireKey, unthrottle_key_X + 2*bottom_key_spacing * scale, unthrottle_key_Y - 60 * scale);
	initTouchPos(g_PConfig.touchAnalogRotationCCWKey, unthrottle_key_X, unthrottle_key_Y - 120 * scale);
	initTouchPos(g_PConfig.touchAnalogRotationCWKey, unthrottle_key_X + bottom_key_spacing * scale, unthrottle_key_Y - 120 * scale);
	...
	initTouchPos(g_PConfig.touchLKey, l_key_X, l_key_Y);

	int r_key_X = xres - 60 * scale;
	int r_key_Y = l_key_Y;
	initTouchPos(g_PConfig.touchRKey, r_key_X, r_key_Y);

	//Combo key
	int combo_key_X = halfW + bottom_key_spacing * scale * 1.2f;
	int combo_key_Y = yres / 2;
	initTouchPos(g_PConfig.touchCombo0, combo_key_X, combo_key_Y);

	int combo1_key_X = halfW + bottom_key_spacing * scale * 2.2f;
	int combo1_key_Y = yres / 2;
	initTouchPos(g_PConfig.touchCombo1, combo1_key_X, combo1_key_Y);

	int combo2_key_X = halfW + bottom_key_spacing * scale * 3.2f;
	int combo2_key_Y = yres / 2;
	initTouchPos(g_PConfig.touchCombo2, combo2_key_X, combo2_key_Y);

	int combo3_key_X = halfW + bottom_key_spacing * scale * 1.2f;
	int combo3_key_Y = yres / 3;
	initTouchPos(g_PConfig.touchCombo3, combo3_key_X, combo3_key_Y);
...
UI::ViewGroup *CreatePadLayout(float xres, float yres, bool *pause) {
	...
	const float actionButtonSpacing = g_PConfig.fActionButtonSpacing * baseActionButtonSpacing;
	...
	const ImageID roundImage = g_PConfig.iTouchButtonStyle ? ImageID("I_ROUND_LINE") : ImageID("I_ROUND");

	const ImageID rectImage = g_PConfig.iTouchButtonStyle ? ImageID("I_RECT_LINE") : ImageID("I_RECT");
	const ImageID shoulderImage = g_PConfig.iTouchButtonStyle ? ImageID("I_SHOULDER_LINE") : ImageID("I_SHOULDER");
	const ImageID dirImage = g_PConfig.iTouchButtonStyle ? ImageID("I_DIR_LINE") : ImageID("I_DIR");
	const ImageID stickImage = g_PConfig.iTouchButtonStyle ? ImageID("I_STICK_LINE") : ImageID("I_STICK");
	const ImageID stickBg = g_PConfig.iTouchButtonStyle ? ImageID("I_STICK_BG_LINE") : ImageID("I_STICK_BG");
	...
	if (!System_GetPropertyBool(SYSPROP_HAS_BACK_BUTTON) || g_PConfig.bShowTouchPause) {
	...
	if (g_PConfig.bShowTouchCircle)
		addPSPButton(CTRL_CIRCLE, roundImage, ImageID("I_ROUND"), ImageID("I_CIRCLE"), g_PConfig.touchActionButtonCenter, circleOffset);
	if (g_PConfig.bShowTouchCross)
		addPSPButton(CTRL_CROSS, roundImage, ImageID("I_ROUND"), ImageID("I_CROSS"), g_PConfig.touchActionButtonCenter, crossOffset);
	if (g_PConfig.bShowTouchTriangle)
		addPSPButton(CTRL_TRIANGLE, roundImage, ImageID("I_ROUND"), ImageID("I_TRIANGLE"), g_PConfig.touchActionButtonCenter, triangleOffset);
	if (g_PConfig.bShowTouchSquare)
		addPSPButton(CTRL_SQUARE, roundImage, ImageID("I_ROUND"), ImageID("I_SQUARE"), g_PConfig.touchActionButtonCenter, squareOffset);

	addPSPButton(CTRL_START, rectImage, ImageID("I_RECT"), ImageID("I_START"), g_PConfig.touchStartKey);
	addPSPButton(CTRL_SELECT, rectImage, ImageID("I_RECT"), ImageID("I_SELECT"), g_PConfig.touchSelectKey);

	BoolButton *unthrottle = addBoolButton(&PSP_CoreParameter().unthrottle, rectImage, ImageID("I_RECT"), ImageID("I_ARROW"), g_PConfig.touchUnthrottleKey);
	if (unthrottle) {
	...
	if (g_PConfig.touchRapidFireKey.show) {
		auto rapidFire = root->Add(new RapidFireButton(rectImage, ImageID("I_RECT"), ImageID("I_ARROW"), g_PConfig.touchRapidFireKey.scale, buttonLayoutParams(g_PConfig.touchRapidFireKey)));
		rapidFire->SetAngle(90.0f, 180.0f);
	}

	if (g_PConfig.touchAnalogRotationCWKey.show) {
		auto analogRotationCC = root->Add(new AnalogRotationButton(true, rectImage, ImageID("I_RECT"), ImageID("I_ARROW"), g_PConfig.touchAnalogRotationCWKey.scale, buttonLayoutParams(g_PConfig.touchAnalogRotationCWKey)));
		analogRotationCC->SetAngle(190.0f, 180.0f);
	}

	if (g_PConfig.touchAnalogRotationCCWKey.show) {
		auto analogRotationCCW = root->Add(new AnalogRotationButton(false, rectImage, ImageID("I_RECT"), ImageID("I_ARROW"), g_PConfig.touchAnalogRotationCCWKey.scale, buttonLayoutParams(g_PConfig.touchAnalogRotationCCWKey)));
		analogRotationCCW->SetAngle(350.0f, 180.0f);
	}

	FPSLimitButton *speed1 = addFPSLimitButton(FPSLimit::CUSTOM1, rectImage, ImageID("I_RECT"), ImageID("I_ARROW"), g_PConfig.touchSpeed1Key);
	if (speed1)
		speed1->SetAngle(170.0f, 180.0f);
	FPSLimitButton *speed2 = addFPSLimitButton(FPSLimit::CUSTOM2, rectImage, ImageID("I_RECT"), ImageID("I_ARROW"), g_PConfig.touchSpeed2Key);
	if (speed2)
		speed2->SetAngle(190.0f, 180.0f);

	addPSPButton(CTRL_LTRIGGER, shoulderImage, ImageID("I_SHOULDER"), ImageID("I_L"), g_PConfig.touchLKey);
	PSPButton *rTrigger = addPSPButton(CTRL_RTRIGGER, shoulderImage, ImageID("I_SHOULDER"), ImageID("I_R"), g_PConfig.touchRKey);
	if (rTrigger)
		rTrigger->FlipImageH(true);

	if (g_PConfig.touchDpad.show)
		root->Add(new PSPDpad(dirImage, ImageID("I_DIR"), ImageID("I_ARROW"), g_PConfig.touchDpad.scale, g_PConfig.fDpadSpacing, buttonLayoutParams(g_PConfig.touchDpad)));

	if (g_PConfig.touchAnalogStick.show)
		root->Add(new PSPStick(stickBg, stickImage, ImageID("I_STICK"), 0, g_PConfig.touchAnalogStick.scale, buttonLayoutParams(g_PConfig.touchAnalogStick)));

	if (g_PConfig.touchRightAnalogStick.show) {
		if (g_PConfig.bRightAnalogCustom)
			root->Add(new PSPCustomStick(stickBg, stickImage, ImageID("I_STICK"), g_PConfig.touchRightAnalogStick.scale, buttonLayoutParams(g_PConfig.touchRightAnalogStick)));
		else
			root->Add(new PSPStick(stickBg, stickImage, ImageID("I_STICK"), 1, g_PConfig.touchRightAnalogStick.scale, buttonLayoutParams(g_PConfig.touchRightAnalogStick)));
	}

	addComboKey(g_PConfig.iCombokey0, g_PConfig.bComboToggle0, roundImage, ImageID("I_ROUND"), comboKeyImages[0], g_PConfig.touchCombo0);
	addComboKey(g_PConfig.iCombokey1, g_PConfig.bComboToggle1, roundImage, ImageID("I_ROUND"), comboKeyImages[1], g_PConfig.touchCombo1);
	addComboKey(g_PConfig.iCombokey2, g_PConfig.bComboToggle2, roundImage, ImageID("I_ROUND"), comboKeyImages[2], g_PConfig.touchCombo2);
	addComboKey(g_PConfig.iCombokey3, g_PConfig.bComboToggle3, roundImage, ImageID("I_ROUND"), comboKeyImages[3], g_PConfig.touchCombo3);
	addComboKey(g_PConfig.iCombokey4, g_PConfig.bComboToggle4, roundImage, ImageID("I_ROUND"), comboKeyImages[4], g_PConfig.touchCombo4);
