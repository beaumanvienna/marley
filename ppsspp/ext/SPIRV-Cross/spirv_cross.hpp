...
{
	std::string name;
	Pspv::ExecutionModel execution_model;
};
...
public:
	...
	void set_decoration(uint32_t id, Pspv::Decoration decoration, uint32_t argument = 0);
	void set_decoration_string(uint32_t id, Pspv::Decoration decoration, const std::string &argument);
	...
	// I.e. (1ull << Pspv::DecorationFoo) | (1ull << Pspv::DecorationBar)
	const Bitset &get_decoration_bitset(uint32_t id) const;

	// Returns whether the decoration has been applied to the ID.
	bool has_decoration(uint32_t id, Pspv::Decoration decoration) const;

	// Gets the value for decorations which take arguments.
	// If the decoration is a boolean (i.e. Pspv::DecorationNonWritable),
	// 1 will be returned.
	// If decoration doesn't exist or decoration is not recognized,
	// 0 will be returned.
	uint32_t get_decoration(uint32_t id, Pspv::Decoration decoration) const;
	const std::string &get_decoration_string(uint32_t id, Pspv::Decoration decoration) const;

	// Removes the decoration for an ID.
	void unset_decoration(uint32_t id, Pspv::Decoration decoration);
	...
	Pspv::StorageClass get_storage_class(uint32_t id) const;
	...
	uint32_t get_member_decoration(uint32_t id, uint32_t index, Pspv::Decoration decoration) const;
	const std::string &get_member_decoration_string(uint32_t id, uint32_t index, Pspv::Decoration decoration) const;
	...
	bool has_member_decoration(uint32_t id, uint32_t index, Pspv::Decoration decoration) const;

	// Similar to set_decoration, but for struct members.
	void set_member_decoration(uint32_t id, uint32_t index, Pspv::Decoration decoration, uint32_t argument = 0);
	void set_member_decoration_string(uint32_t id, uint32_t index, Pspv::Decoration decoration,
	                                  const std::string &argument);

	// Unsets a member decoration, similar to unset_decoration.
	void unset_member_decoration(uint32_t id, uint32_t index, Pspv::Decoration decoration);
	...
	void set_entry_point(const std::string &entry, Pspv::ExecutionModel execution_model);
	...
	void rename_entry_point(const std::string &old_name, const std::string &new_name,
	                        Pspv::ExecutionModel execution_model);
	const SPIREntryPoint &get_entry_point(const std::string &name, Pspv::ExecutionModel execution_model) const;
	SPIREntryPoint &get_entry_point(const std::string &name, Pspv::ExecutionModel execution_model);
	const std::string &get_cleansed_entry_point_name(const std::string &name,
	                                                 Pspv::ExecutionModel execution_model) const;

	// Query and modify OpExecutionMode.
	const Bitset &get_execution_mode_bitset() const;

	void unset_execution_mode(Pspv::ExecutionMode mode);
	void set_execution_mode(Pspv::ExecutionMode mode, uint32_t arg0 = 0, uint32_t arg1 = 0, uint32_t arg2 = 0);
	...
	uint32_t get_execution_mode_argument(Pspv::ExecutionMode mode, uint32_t index = 0) const;
	Pspv::ExecutionModel get_execution_model() const;
	...
	bool get_binary_offset_for_decoration(uint32_t id, Pspv::Decoration decoration, uint32_t &word_offset) const;
	...
	const SmallVector<Pspv::Capability> &get_declared_capabilities() const;
...
protected:
	...
	static bool is_tessellation_shader(Pspv::ExecutionModel model);
	...
	bool is_member_builtin(const SPIRType &type, uint32_t index, Pspv::BuiltIn *builtin) const;
	...
	struct OpcodeHandler
	{
		...
		virtual bool handle(Pspv::Op opcode, const uint32_t *args, uint32_t length) = 0;
	...
	struct ActiveBuiltinHandler : OpcodeHandler
	{
		...
		void handle_builtin(const SPIRType &type, Pspv::BuiltIn builtin, const Bitset &decoration_flags);
	};
	...
	bool has_active_builtin(Pspv::BuiltIn builtin, Pspv::StorageClass storage);
	...
	struct CFGBuilder : OpcodeHandler
	{
		...
		bool handle(Pspv::Op op, const uint32_t *args, uint32_t length) override;
		Compiler &compiler;
		std::unordered_map<uint32_t, std::unique_ptr<CFG>> function_cfgs;
	};
	...
	bool instruction_to_result_type(uint32_t &result_type, uint32_t &result_id, Pspv::Op op, const uint32_t *args,
	                                uint32_t length);

	Bitset combined_decoration_for_member(const SPIRType &type, uint32_t index) const;
	static bool is_desktop_only_format(Pspv::ImageFormat format);
