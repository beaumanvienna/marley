...
void CreateImage(VulkanContext *vulkan, VkCommandBuffer cmd, VKRImage &img, int width, int height, VkFormat format, VkImageLayout initialLayout, bool color) {
	...
	res = PPvkCreateImageView(vulkan->GetDevice(), &ivci, nullptr, &img.imageView);
	assert(res == VK_SUCCESS);
...
VulkanRenderManager::VulkanRenderManager(VulkanContext *vulkan) : vulkan_(vulkan), queueRunner_(vulkan) {
	...
	for (int i = 0; i < vulkan_->GetInflightFrames(); i++) {
		...
		res = PvkCreateQueryPool(vulkan_->GetDevice(), &query_ci, nullptr, &frameData_[i].timestampQueryPool_);
	}
...
void VulkanRenderManager::CreateBackbuffers() {
	...
	for (uint32_t i = 0; i < swapchainImageCount_; i++) {
		...
		res = PPvkCreateImageView(vulkan_->GetDevice(), &color_image_view, nullptr, &sc_buffer.view);
...
VulkanRenderManager::~VulkanRenderManager() {
	...
	for (int i = 0; i < vulkan_->GetInflightFrames(); i++) {
		...
		PvkDestroyQueryPool(device, frameData_[i].timestampQueryPool_, nullptr);
	}
...
void VulkanRenderManager::BeginFrame(bool enableProfiling) {
	...
	if (frameData_->profilingEnabled_) {
		...
		PvkCmdResetQueryPool(initCmd, frameData.timestampQueryPool_, 0, MAX_TIMESTAMP_QUERIES);
		PvkCmdWriteTimestamp(initCmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, frameData.timestampQueryPool_, 0);
	}
}
...
bool VulkanRenderManager::InitDepthStencilBuffer(VkCommandBuffer cmd) {
	...
	res = PPvkCreateImageView(device, &depth_view_info, NULL, &depth_.view);
	assert(res == VK_SUCCESS);
	if (res != VK_SUCCESS)
		return false;
...
void VulkanRenderManager::BeginSubmitFrame(int frame) {
	...
	if (!frameData.hasBegun) {
		...
		if (res == VK_SUBOPTIMAL_KHR) {
			...
			_assert_msg_(G3D, res == VK_SUCCESS, "PvkAcquireNextImageKHR failed! result=%s", VulkanResultToString(res));
		}
		...
		_assert_msg_(G3D, res == VK_SUCCESS, "PvkBeginCommandBuffer failed! result=%s", VulkanResultToString(res));
...
void VulkanRenderManager::Submit(int frame, bool triggerFence) {
	...
	if (frameData.hasInitCommands) {
		if (frameData.profilingEnabled_ && triggerFence) {
			...
			PvkCmdWriteTimestamp(frameData.initCmd, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, frameData.timestampQueryPool_, 1);
		}
		VkResult res = PvkEndCommandBuffer(frameData.initCmd);
		_assert_msg_(G3D, res == VK_SUCCESS, "PvkEndCommandBuffer failed (init)! result=%s", VulkanResultToString(res));
	}

	VkResult res = PvkEndCommandBuffer(frameData.mainCmd);
	_assert_msg_(G3D, res == VK_SUCCESS, "PvkEndCommandBuffer failed (main)! result=%s", VulkanResultToString(res));
	...
	if (frameData.hasInitCommands) {
		...
		if (splitSubmit_) {
			...
			if (res == VK_ERROR_DEVICE_LOST) {
				...
				_assert_msg_(G3D, res == VK_SUCCESS, "PvkQueueSubmit failed (init)! result=%s", VulkanResultToString(res));
			}
			numCmdBufs = 0;
		}
	}
	...
	res = PvkQueueSubmit(vulkan_->GetGraphicsQueue(), 1, &submit_info, triggerFence ? frameData.fence : VK_NULL_HANDLE);
	if (res == VK_ERROR_DEVICE_LOST) {
#ifdef _WIN32
		_assert_msg_(G3D, false, "Lost the Vulkan device! If this happens again, switch Graphics Backend from Vulkan to Direct3D11");
#else
		...
		_assert_msg_(G3D, res == VK_SUCCESS, "PvkQueueSubmit failed (main, split=%d)! result=%s", (int)splitSubmit_, VulkanResultToString(res));
	}
...
void VulkanRenderManager::EndSubmitFrame(int frame) {
	...
	if (!frameData.skipSwap) {
		...
		if (res == VK_ERROR_OUT_OF_DATE_KHR || res == VK_SUBOPTIMAL_KHR) {
			...
			_assert_msg_(G3D, res == VK_SUCCESS, "PvkQueuePresentKHR failed! result=%s", VulkanResultToString(res));
		}
	} else {
