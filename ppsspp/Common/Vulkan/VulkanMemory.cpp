...
bool VulkanPushBuffer::AddBuffer() {
	...
	if (VK_SUCCESS != res) {
		_assert_msg_(false, "PvkCreateBuffer failed! result=%d", (int)res);
		return false;
	}
	...
	if (VK_SUCCESS != res) {
		_assert_msg_(false, "PvkAllocateMemory failed! size=%d result=%d", (int)reqs.size, (int)res);
...
void VulkanPushBuffer::Unmap() {
	...
	if ((memoryPropertyMask_ & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) == 0) {
		...
		PvkFlushMappedMemoryRanges(vulkan_->GetDevice(), 1, &range);
	}
