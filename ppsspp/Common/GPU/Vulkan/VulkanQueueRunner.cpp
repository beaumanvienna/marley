...
void VulkanQueueRunner::ResizeReadbackBuffer(VkDeviceSize requiredSize) {
	...
	VkResult res = PvkCreateBuffer(device, &buf, nullptr, &readbackBuffer_);
	...
	res = PvkAllocateMemory(device, &allocInfo, nullptr, &readbackMemory_);
	if (res != VK_SUCCESS) {
...
void TransitionToOptimal(VkCommandBuffer cmd, VkImage colorImage, VkImageLayout colorLayout, VkImage depthStencilImage, VkImageLayout depthStencilLayout) {
	...
	if (barrierCount) {
		PvkCmdPipelineBarrier(cmd, srcStageMask, dstStageMask, VK_DEPENDENCY_BY_REGION_BIT, 0, nullptr, 0, nullptr, barrierCount, barrier);
	}
}
...
void VulkanQueueRunner::PerformRenderPass(const VKRStep &step, VkCommandBuffer cmd) {
	...
	if (step.render.framebuffer) {
		...
		if (stage) {
			PvkCmdPipelineBarrier(cmd, stage, stage, 0, 0, nullptr, 0, nullptr, n, barriers);
