...
void VulkanContext::ChooseDevice(int physical_device) {
	...
	PvkGetPhysicalDeviceFormatProperties(physical_devices_[physical_device_], deviceInfo_.preferredDepthStencilFormat, &preferredProps);
...
void VulkanContext::SetDebugNameImpl(uint64_t handle, VkObjectType type, const char *name) {
	...
	PvkSetDebugUtilsObjectNameEXT(device_, &info);
}
...
VkResult VulkanContext::ReinitSurface() {
	...
	case WINDOWSYSTEM_XLIB:
	{
		...
		retval = PvkCreateXlibSurfaceKHR(instance_, &xlib, nullptr, &surface_);
		break;
	}
#endif
#if defined(VK_USE_PLATFORM_XCB_KHR)
...
void VulkanDeleteList::PerformDeletes(VkDevice device) {
	...
	for (auto &bufView : bufferViews_) {
		PvkDestroyBufferView(device, bufView, nullptr);
	}
	bufferViews_.clear();
	...
	for (auto &imageView : imageViews_) {
		PvkDestroyImageView(device, imageView, nullptr);
	}
	imageViews_.clear();
	...
	for (auto &pcache : pipelineCaches_) {
		PvkDestroyPipelineCache(device, pcache, nullptr);
	}
	...
	for (auto &pipeLayout : pipelineLayouts_) {
		PvkDestroyPipelineLayout(device, pipeLayout, nullptr);
	}
