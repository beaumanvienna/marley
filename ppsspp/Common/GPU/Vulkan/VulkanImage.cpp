...
bool VulkanTexture::CreateDirect(VkCommandBuffer cmd, VulkanDeviceAllocator *allocator, int w, int h, int numMips, VkFormat format, VkImageLayout initialLayout, VkImageUsageFlags usage, const VkComponentMapping *mapping) {
	...
	if (allocator && !dedicatedAllocation) {
		...
		if (res != VK_SUCCESS) {
			ERROR_LOG(G3D, "PvkAllocateMemory failed: %s", VulkanResultToString(res));
	...
	if (res != VK_SUCCESS) {
		ERROR_LOG(G3D, "PvkBindImageMemory failed: %s", VulkanResultToString(res));
	...
	res = PvkCreateImageView(vulkan_->GetDevice(), &view_info, NULL, &view_);
	if (res != VK_SUCCESS) {
...
void VulkanTexture::UploadMip(VkCommandBuffer cmd, int mip, int mipWidth, int mipHeight, VkBuffer buffer, uint32_t offset, size_t rowLength) {
	...
	PvkCmdCopyBufferToImage(cmd, buffer, image_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy_region);
}
...
void VulkanTexture::ClearMip(VkCommandBuffer cmd, int mip, uint32_t value) {
	...
	range.levelCount = 1;
	PvkCmdClearColorImage(cmd, image_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearVal, 1, &range);
}
...
VkImageView VulkanTexture::CreateViewForMip(int mip) {
	...
	view_info.subresourceRange.layerCount = 1;
	VkImageView view;
	VkResult res = PvkCreateImageView(vulkan_->GetDevice(), &view_info, NULL, &view);
